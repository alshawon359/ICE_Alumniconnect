...
def add_security_headers(response):
    ...
    # CSP can break inline scripts/styles in existing pages; keep it opt-in via environment.
    csp = (os.getenv('CONTENT_SECURITY_POLICY') or '').strip()
    if csp:
...
_db_ready_lock = Lock()
ADMIN_TOKEN_SALT = 'admin-auth-v1'
ADMIN_TOKEN_MAX_AGE = int(os.getenv('ADMIN_TOKEN_MAX_AGE', str(7 * 24 * 60 * 60)))
    ...
    try:
        ...
        create_stmts = [
            """CREATE TABLE IF NOT EXISTS admin_accounts (
                ...
                role          ENUM('super_admin','co_admin') NOT NULL DEFAULT 'co_admin',
        ...
        try:
            _ensure_bootstrap_admin(cur)
        except Exception as e:
            print(f"[DB MIGRATION] Bootstrap admin warning (non-fatal): {e}")
        ...
        # Add approved_by and approved_at columns to alumni if missing
        try:
            cur.execute("ALTER TABLE alumni ADD COLUMN approved_by VARCHAR(120) DEFAULT NULL")
        except pymysql_err.OperationalError as e:
            if e.args[0] != 1060:
                print(f"[DB MIGRATION] Note (non-fatal): {e}")

        try:
            cur.execute("ALTER TABLE alumni ADD COLUMN approved_at DATETIME DEFAULT NULL")
        except pymysql_err.OperationalError as e:
            if e.args[0] != 1060:
                print(f"[DB MIGRATION] Note (non-fatal): {e}")
        ...
        # Add optional training time column if missing
        try:
            cur.execute("ALTER TABLE trainings ADD COLUMN `time` TIME DEFAULT NULL")
        except pymysql_err.OperationalError as e:
            if e.args[0] != 1060:  # Column already exists is OK
                print(f"[DB MIGRATION] Note (non-fatal): {e}")
        ...
        # Backward-compatible admin account columns (for databases created before role-based admin release)
        admin_account_alters = [
            "ALTER TABLE admin_accounts ADD COLUMN name VARCHAR(120) DEFAULT NULL AFTER username",
            "ALTER TABLE admin_accounts ADD COLUMN role ENUM('super_admin','co_admin') NOT NULL DEFAULT 'co_admin'",
            "ALTER TABLE admin_accounts ADD COLUMN is_active TINYINT(1) NOT NULL DEFAULT 1",
            "ALTER TABLE admin_accounts ADD COLUMN created_by VARCHAR(60) DEFAULT NULL",
            "ALTER TABLE admin_accounts ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP",
            "ALTER TABLE admin_accounts ADD COLUMN last_login_at DATETIME DEFAULT NULL",
        ]
        for stmt in admin_account_alters:
            try:
                cur.execute(stmt)
            except pymysql_err.OperationalError as e:
                if e.args[0] != 1060:  # Column already exists
                    print(f"[DB MIGRATION] admin_accounts ALTER note (non-fatal): {e}")
...
def _get_admin_token_serializer():
    return URLSafeTimedSerializer(config.SECRET_KEY, salt=ADMIN_TOKEN_SALT)
...
def _encode_admin_token(admin_row):
    payload = {
        'admin_id': int(admin_row['id']),
        'username': admin_row['username'],
        'role': admin_row['role'],
    }
    return _get_admin_token_serializer().dumps(payload)


def _decode_admin_token(token):
    try:
        return _get_admin_token_serializer().loads(token, max_age=ADMIN_TOKEN_MAX_AGE)
...
def _ensure_bootstrap_admin(cursor=None):
    ...
    try:
        ...
        cur.execute("SELECT id FROM admin_accounts WHERE role='super_admin' LIMIT 1")
        if cur.fetchone():
            return

        cur.execute("SELECT id FROM admin_accounts WHERE username=%s LIMIT 1", (username,))
        existing_admin = cur.fetchone()
        existing_admin_id = None
        if existing_admin:
            if isinstance(existing_admin, dict):
                existing_admin_id = existing_admin.get('id')
            elif isinstance(existing_admin, (tuple, list)) and existing_admin:
                existing_admin_id = existing_admin[0]
        if existing_admin:
            cur.execute(
                "UPDATE admin_accounts SET role='super_admin', is_active=1 WHERE id=%s",
                (existing_admin_id,),
            )
            if conn is not None:
                conn.commit()
        ...
        cur.execute(
            "INSERT INTO admin_accounts (username, name, password_hash, role, is_active, created_by) VALUES (%s, %s, %s, 'super_admin', 1, %s)",
            (username, 'Super Admin', generate_password_hash(password), username),
        )
        if conn is not None:
            conn.commit()
    finally:
        if own_cursor and cur is not None:
            try:
                cur.close()
            except Exception:
                pass
...
def _require_admin_auth(allowed_roles=None):
    ...
    if not payload:
        return None, (jsonify({'success': False, 'message': 'Admin session expired or invalid'}), 401)

    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute(
            "SELECT id, username, name, role, is_active, created_by, created_at, updated_at, last_login_at FROM admin_accounts WHERE id=%s LIMIT 1",
            (payload.get('admin_id'),),
        )
        admin_row = cur.fetchone()
        if not admin_row or not int(admin_row.get('is_active') or 0):
            return None, (jsonify({'success': False, 'message': 'Admin account is disabled'}), 403)

        if allowed_roles and admin_row.get('role') not in allowed_roles:
            return None, (jsonify({'success': False, 'message': 'Insufficient permissions'}), 403)

        return admin_row, None
    finally:
        cur.close()
...
def brevo_smtp_configured():
    return bool(
        config.BREVO_SMTP_HOST
        and config.BREVO_SMTP_PORT
        and config.BREVO_EMAIL
...
def active_mail_provider():
    ...
    if provider in {'smtp', 'brevo', 'brevo_smtp'}:
        return provider
    # In auto mode prefer Brevo when available so production uses the intended provider first.
    if brevo_configured():
        return 'brevo'
    if brevo_smtp_configured():
        return 'brevo_smtp'
    if smtp_configured():
        return 'smtp'
    return 'local'
...
def _send_email_via_local_outbox(recipients, subject, plain_content, html_content):
    outbox_dir = os.path.join(os.path.dirname(__file__), 'email-outbox')
    os.makedirs(outbox_dir, exist_ok=True)

    sent_count = 0
    failed_count = 0
    errors = []
    sender_email = config.SMTP_FROM_EMAIL if config.SMTP_FROM_EMAIL else 'no-reply@localhost'
    sender_name = config.SMTP_FROM_NAME if config.SMTP_FROM_NAME else 'AlumniConnect Admin'
    ...
    for index, email in enumerate(recipients, start=1):
        try:
            ...
            safe_subject = re.sub(r'[^A-Za-z0-9._-]+', '_', subject or 'message').strip('_')[:60] or 'message'
            safe_email = re.sub(r'[^A-Za-z0-9._-]+', '_', email).strip('_')[:60] or 'recipient'
            file_name = f"{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{index}_{safe_subject}_{safe_email}.eml"
...
def send_email(to_emails, subject, message, preheader='', cta_text='Visit AlumniConnect'):
    ...
    if regular_recipients:
        if provider == 'local':
            result = _send_email_via_local_outbox(regular_recipients, subject, plain_content, html_content)
            total_sent += result.get('sent', 0)
            total_failed += result.get('failed', 0)
            all_errors.extend(result.get('errors', []))
            delivery_transport = result.get('transport') or 'local'
        elif provider == 'brevo':
            try:
            ...
            try:
                result = _send_email_via_brevo_smtp(regular_recipients, subject, plain_content, html_content)
            except Exception as ex:
                total_failed += len(regular_recipients)
                all_errors.extend([
                    {'email': email, 'error': f'Brevo SMTP error: {str(ex)}'}
                    for email in regular_recipients
                ])
            else:
                total_sent += result.get('sent', 0)
    ...
    response = {'sent': total_sent, 'failed': total_failed, 'errors': all_errors}
    if delivery_transport:
        response['transport'] = delivery_transport
    return response


def _send_email_via_brevo_with_fallback(recipients, subject, plain_content, html_content):
    if not brevo_configured():
        error_msg = 'Brevo is not configured. Set BREVO_API_KEY and SMTP_FROM_EMAIL in backend/.env.'
        return {
            'sent': 0,
            'failed': len(recipients),
            'errors': [{'email': email, 'error': error_msg} for email in recipients]
        }
...
def _send_email_via_brevo_smtp(recipients, subject, plain_content, html_content):
    if not brevo_smtp_configured():
        raise RuntimeError('Brevo SMTP is not configured. Set BREVO_EMAIL and BREVO_API_KEY.')

    sent_count = 0
    failed_count = 0
    errors = []

    with smtplib.SMTP(config.BREVO_SMTP_HOST, config.BREVO_SMTP_PORT, timeout=20) as server:
        if config.BREVO_SMTP_USE_TLS:
            server.starttls()
        server.login(config.BREVO_EMAIL, config.BREVO_API_KEY)
    ...
    return {'sent': sent_count, 'failed': failed_count, 'errors': errors, 'transport': 'brevo_smtp'}
...
def admin_login():
    try:
        ...
        try:
            cur.execute(
                "SELECT id, username, password_hash, role, is_active FROM admin_accounts WHERE username=%s LIMIT 1",
                (username,),
            )
            admin_row = cur.fetchone()
            if not admin_row or not int(admin_row.get('is_active') or 0):
            ...
            if not check_password_hash(admin_row['password_hash'], password):
                return jsonify({'success': False, 'message': 'Invalid username or password'}), 401

            try:
                cur.execute(
                    "UPDATE admin_accounts SET last_login_at=NOW() WHERE id=%s",
                    (admin_row['id'],),
                )
            except pymysql_err.OperationalError as e:
                # Allow login if legacy schema is temporarily missing this optional audit column.
                print(f"[ADMIN LOGIN] last_login_at update skipped: {e}")
            conn.commit()
            token = _encode_admin_token(admin_row)
            return jsonify({
                'success': True,
...
def admin_accounts():
    admin_user, error_response = _require_admin_auth(allowed_roles={'super_admin'})
    if error_response:
    ...
    try:
        if request.method == 'GET':
            cur.execute(
                """
                SELECT id, username, name, role, is_active, created_by, created_at, updated_at, last_login_at
                FROM admin_accounts
                ORDER BY
        ...
        username = (data.get('username') or '').strip()
        password = (data.get('password') or '').strip()
        role = (data.get('role') or 'co_admin').strip().lower()

        if role != 'co_admin':
            return jsonify({'success': False, 'message': 'Only co-admin accounts can be created here'}), 400
        if len(name) < 2:
            return jsonify({'success': False, 'message': 'Co-admin name must be at least 2 characters'}), 400
        if len(username) < 3:
        ...
        cur.execute(
            "INSERT INTO admin_accounts (name, username, password_hash, role, is_active, created_by) VALUES (%s, %s, %s, 'co_admin', 1, %s)",
            (name, username, generate_password_hash(password), admin_user['username']),
        )
        conn.commit()
...
@app.route('/api/admin-accounts/<int:admin_id>', methods=['DELETE', 'PATCH'])
def delete_admin_account(admin_id):
    ...
    cur = conn.cursor()
    try:
        # Check if target admin exists and is a co_admin
        cur.execute(
            "SELECT id, role, username, name FROM admin_accounts WHERE id=%s LIMIT 1",
            (admin_id,)
        )
        ...
        if target_admin['role'] != 'co_admin':
            return jsonify({'success': False, 'message': 'Cannot delete super admin accounts'}), 400
        ...
        if request.method == 'PATCH':
            ...
            cur.execute(
                "UPDATE admin_accounts SET name=%s, updated_at=NOW() WHERE id=%s",
                (name, admin_id),
            )
            conn.commit()
            return jsonify({
                'success': True,
                'message': 'Co-admin name updated successfully',
                'admin': {
                    'id': target_admin['id'],
        ...
        # Soft delete: set is_active = 0 to prevent login
        cur.execute(
            "UPDATE admin_accounts SET is_active = 0, updated_at = NOW() WHERE id=%s",
            (admin_id,)
        )
        conn.commit()
        return jsonify({
            'success': True,
            'message': f"Co-admin '{target_admin['username']}' has been deleted and cannot login anymore"
        }), 200
    except Exception as e:
        print(f"[ADMIN DELETE ERROR]: {e}")
        return jsonify({'success': False, 'message': 'Delete error occurred'}), 500
    finally:
        cur.close()
...
def request_forgot_password_otp():
    ...
    try:
        ...
        if int(result.get('failed', 0)) > 0:
            email_errors = result.get('errors', [])
            error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
            email_warning = f"OTP generated but email delivery failed: {error_msg[:100]}"
    except Exception as ex:
        email_warning = f"OTP generated but email service error: {str(ex)[:100]}"
...
def reset_password_with_otp():
    ...
    try:
        ...
        if int(result.get('failed', 0)) > 0:
            email_errors = result.get('errors', [])
            error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
            email_warning = f"Password changed but email delivery failed: {error_msg[:100]}"
    except Exception as ex:
        email_warning = f"Password changed but email service error: {str(ex)[:100]}"
...
def register():
    ...
    try:
        ...
        if existing:
            ...
            warning = None
            try:
                result = send_registration_received_email(data.get('name'), email, user_type)
                if int(result.get('failed', 0)) > 0:
                    email_errors = result.get('errors', [])
                    error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
                    warning = f"Registration submitted but email delivery failed: {error_msg[:100]}"
            except Exception as ex:
                warning = f"Registration submitted but email service error: {str(ex)[:100]}"
            return jsonify({
                'success': True,
                'id': existing['id'],
...
def get_students():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT id,name,email,phone,address,department,student_id,session,company,designation,current_job_start_date,higher_study,bio,research_interests,extracurricular,linkedin,github,twitter,website,photo,status,hall_name,approved_by,approved_at,created_at FROM alumni WHERE status='approved' AND user_type='student' ORDER BY name")
    rows = cur.fetchall()
    cur.close()
    for r in rows:
        r['photo_url'] = build_upload_url(r.get('photo') or _get_default_photo_filename())
...
def approve(aid):
    admin_user, error_response = _require_admin_auth()
    if error_response:
        return error_response
    ...
    # On approval, capture admin name and approval timestamp
    admin_name = admin_user.get('name') or admin_user.get('username', 'Admin')
    cur.execute(
        "UPDATE alumni SET status='approved', is_manually_added=0, approved_by=%s, approved_at=NOW() WHERE id=%s",
        (admin_name, aid)
    )
    conn.commit()
    cur.close()
    ...
    if person and person.get('email'):
        try:
            ...
            if int(result.get('failed', 0)) > 0:
                email_errors = result.get('errors', [])
                error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
                warning = f"Account approved but email delivery failed: {error_msg[:100]}"
        except Exception as ex:
            warning = f"Account approved but email service error: {str(ex)[:100]}"

    response = {'success': True}
    if warning:
        response['email_warning'] = warning
    return jsonify(response)
...
def reject(aid):
    ...
    conn = get_db()
    cur = conn.cursor()
    # Capture admin name and rejection timestamp
    admin_name = admin_user.get('name') or admin_user.get('username', 'Admin')
    cur.execute("UPDATE alumni SET status='rejected', approved_by=%s, approved_at=NOW() WHERE id=%s", (admin_name, aid))
    conn.commit()
    cur.close()
    return jsonify({'success': True})
...
def approve_referral(referral_id):
    ...
    warning = None
    try:
        result = send_referral_invitation_email(referral.get('referred_name'), referral.get('referred_email'))
        if int(result.get('failed', 0)) > 0:
            email_errors = result.get('errors', [])
            error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
            warning = f"Referral approved but email delivery failed: {error_msg[:100]}"
    except Exception as ex:
        warning = f"Referral approved but email service error: {str(ex)[:100]}"
    ...
    if person and person.get('email'):
        try:
            ...
            if int(result.get('failed', 0)) > 0:
                email_errors = result.get('errors', [])
                error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
                warning = f"Account upgraded but email delivery failed: {error_msg[:100]}"
        except Exception as ex:
            warning = f"Account upgraded but email service error: {str(ex)[:100]}"
...
def get_events():
    ...
    for r in rows:
        ...
        if r.get('attachment'):
            r['attachment_url'] = build_upload_url(r.get('attachment'))
    return jsonify(rows)
...
def add_event():
    ...
    # Attachment handling (pdf or image)
    attachment_file = request.files.get('attachment') or request.files.get('event_attachment')
    attachment = None
    if attachment_file and attachment_file.filename:
        stored_attachment = save_uploaded_image(attachment_file)
        if not stored_attachment:
            max_mb = max(1, int(MAX_IMAGE_UPLOAD_BYTES / (1024 * 1024)))
            allowed_types = ', '.join(sorted(ALLOWED_EXTENSIONS))
            return jsonify({'success': False, 'message': f'Invalid attachment file. Allowed types: {allowed_types}. Max size: {max_mb}MB.'}), 400
        attachment = build_upload_url(stored_attachment)

    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute(
            "INSERT INTO events (title, date, event_time, registration_deadline, location, description, fee, payment_account, audience, banner_image, attachment) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (title, date_value, time_value, registration_deadline, location, description,
             fee_value, payment_account, audience, banner_image, attachment)
        )
    except pymysql_err.OperationalError as e:
        if e.args and e.args[0] == 1054:  # Unknown column: keep compatibility with older schema
            try:
                cur.execute(
                    "INSERT INTO events (title, date, event_time, location, description, fee, payment_account, audience, banner_image, attachment) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (title, date_value, time_value, location, description, fee_value, payment_account, audience, banner_image, attachment)
                )
    ...
    try:
        ...
        if int(result.get('failed', 0)) > 0:
            email_errors = result.get('errors', [])
            error_msg = email_errors[0].get('error', 'Email delivery failed') if email_errors else 'Email delivery failed'
            warning = f"Event created but email delivery failed: {error_msg[:100]}"
    except Exception as ex:
        warning = f"Event created but email service error: {str(ex)[:100]}"
...
def send_bulk_email_endpoint():
    ...
    try:
        ...
        transport = (result.get('transport') or '').strip().lower()
        status = 'success' if failed == 0 else ('partial' if sent > 0 else 'failed')
        if transport == 'local' and failed == 0:
            status = 'queued'
        insert_email_log(conn, subject, len(recipients), sent, failed, status, None)
        if transport == 'local' and failed == 0:
            response_message = f'Email queued in the local outbox. Sent: {sent}, Failed: {failed}. Configure SMTP or Brevo for real delivery.'
        elif failed == 0:
            response_message = f'Email sent successfully. Sent: {sent}, Failed: {failed}'
        elif sent > 0:
            response_message = f'Email sent partially. Sent: {sent}, Failed: {failed}. Check the email log for details.'
        else:
            response_message = f'Email send failed. Sent: {sent}, Failed: {failed}. Check SMTP settings and try again.'
...
def send_email_secure_endpoint():
    data = request.get_json(silent=True) or {}
    raw_email = (data.get('email') or '').strip()
    subject = (data.get('subject') or 'AlumniConnect Notification').strip()
    message = (data.get('message') or '').strip()
    preheader = (data.get('preheader') or '').strip()
    cta_text = (data.get('cta_text') or 'Visit AlumniConnect').strip()

    recipients = _normalize_recipient_emails([raw_email])
    if not recipients:
        return jsonify({'success': False, 'message': 'A valid email is required'}), 400
    if not message:
        return jsonify({'success': False, 'message': 'Message is required'}), 400
    if len(message) > 5000:
        return jsonify({'success': False, 'message': 'Message is too long (max 5000 chars)'}), 400
    ...
    try:
        if not brevo_configured():
            return jsonify({
                'success': False,
                'message': 'Brevo is not configured. Set BREVO_API_KEY and SMTP_FROM_EMAIL in backend/.env.',
            }), 503

        result = _send_email_via_brevo_with_fallback(recipients, subject, plain_content, html_content)
        ...
        return jsonify({
            ...
            'transport': result.get('transport') or 'brevo',
        }), 200
    except smtplib.SMTPAuthenticationError:
        # Some Brevo accounts expose API keys before SMTP relay credentials are activated.
        # Keep delivery available by falling back to the Brevo HTTP API.
        try:
            fallback = _send_email_via_brevo(recipients, subject, plain_content, html_content)
            if fallback.get('failed', 0) > 0:
                return jsonify({
                    'success': False,
            ...
            return jsonify({'success': False, 'message': 'Email service authentication failed'}), 502
        ...
        try:
            ...
            return jsonify({'success': False, 'message': 'Email service is temporarily unavailable'}), 502
    except Exception:
        return jsonify({'success': False, 'message': 'Unexpected email delivery error'}), 500
...
@app.route('/api/events/<int:eid>/upload-attachment', methods=['POST', 'OPTIONS'])
@app.route('/api/events/<int:eid>/upload-attachment/', methods=['POST', 'OPTIONS'])
@app.route('/events/<int:eid>/upload-attachment', methods=['POST', 'OPTIONS'])
@app.route('/events/<int:eid>/upload-attachment/', methods=['POST', 'OPTIONS'])
def upload_event_attachment(eid):
    # Handle CORS preflight
    if request.method == 'OPTIONS':
        resp = make_response('', 200)
        resp.headers['Access-Control-Allow-Origin'] = '*'
    ...
    try:
        print(f"[UPLOAD-ATTACH] Headers: {dict(request.headers)}")
        print(f"[UPLOAD-ATTACH] Form keys: {list(request.form.keys())}")
        print(f"[UPLOAD-ATTACH] Files keys: {list(request.files.keys())}")
    except Exception:
        pass

    attach_file = request.files.get('attachment') or request.files.get('event_attachment') or request.files.get('file')
    if not attach_file:
        resp = jsonify({'success': False, 'message': 'Attachment file is required'})
        resp.status_code = 400
        resp.headers['Access-Control-Allow-Origin'] = '*'
        return resp
    ...
    try:
        ...
        saved = save_uploaded_image(attach_file)
        if not saved:
            resp = jsonify({'success': False, 'message': 'Invalid attachment file'})
            resp.status_code = 400
            resp.headers['Access-Control-Allow-Origin'] = '*'
            return resp

        attachment_url = build_upload_url(saved)

        cur.execute('UPDATE events SET attachment=%s WHERE id=%s', (attachment_url, eid))
        conn.commit()
        resp = jsonify({
            'success': True,
...
def alumni_events(aid):
    ...
    for r in rows:
        ...
        if r.get('attachment'): r['attachment_url'] = build_upload_url(r.get('attachment'))
    return jsonify(rows)
...
def get_trainings():
    ...
    for r in rows:
        if r.get('date'): r['date'] = str(r['date'])
        if r.get('time'): r['time'] = str(r['time'])
        if r.get('created_at'): r['created_at'] = str(r['created_at'])
    return jsonify(rows)
...
def add_training():
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO trainings (title, trainer, date, `time`, seats, enrolled, status, fee, payment_account, created_by) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (data.get('title'), data.get('trainer'), data.get('date'),
         data.get('time'), data.get('seats', 30), data.get('enrolled', 0), data.get('status', 'Upcoming'),
...
def update_training(tid):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE trainings SET title=%s, trainer=%s, date=%s, `time`=%s, seats=%s, status=%s, fee=%s, payment_account=%s WHERE id=%s",
        (data.get('title'), data.get('trainer'), data.get('date'),
         data.get('time'),
...
def register_for_training(tid):
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT seats, enrolled, date FROM trainings WHERE id=%s", (tid,))
    t = cur.fetchone()
    if not t:
        cur.close()
        return jsonify({'success': False, 'message': 'Training not found'}), 404
    training_date = t.get('date')
    if training_date and training_date < date.today():
        cur.close()
        return jsonify({'success': False, 'message': 'Training is expired'}), 400
...
def get_training_attendees(tid):
    requester_alumni_id = request.args.get('alumni_id', type=int)
    if not requester_alumni_id:
        return jsonify({'success': False, 'message': 'alumni_id is required'}), 400

    conn = get_db()
    cur = conn.cursor()

    # Only the alumni who created this training can view attendees.
    cur.execute("SELECT created_by FROM trainings WHERE id=%s", (tid,))
    training_row = cur.fetchone()
    if not training_row:
        cur.close()
        return jsonify({'success': False, 'message': 'Training not found'}), 404

    creator_id = training_row.get('created_by')
    if not creator_id or str(creator_id) != str(requester_alumni_id):
        cur.close()
        return jsonify({'success': False, 'message': 'Not authorized to view attendees'}), 403
...
def get_success_stories():
    ...
    try:
        ...
        # Resolve optional viewer identity so we can indicate whether the
        # current viewer has reacted to each story.
        viewer_alumni_id = None
        alumni_id_q = request.args.get('alumni_id', '').strip()
        student_id_q = request.args.get('student_id', '').strip()
        if alumni_id_q:
            try:
                viewer_alumni_id = int(alumni_id_q)
            except Exception:
                viewer_alumni_id = None

        if not viewer_alumni_id and student_id_q:
            try:
                conn = get_db()
                cur = conn.cursor()
                cur.execute('SELECT id FROM alumni WHERE student_id = %s LIMIT 1', (student_id_q,))
                r = cur.fetchone()
                cur.close()
                if r:
                    viewer_alumni_id = r['id'] if isinstance(r, dict) else r[0]
            except Exception:
                viewer_alumni_id = None
        ...
        # Build SQL: include reaction counts and optionally whether viewer reacted
        extra_fields = ", (SELECT COUNT(*) FROM story_reactions r WHERE r.story_id = ss.id) as reaction_count"
        extra_fields += ", (SELECT COUNT(*) FROM story_comments sc WHERE sc.story_id = ss.id) as comment_count"
        params = []
        if viewer_alumni_id:
            extra_fields += ", (SELECT EXISTS(SELECT 1 FROM story_reactions rr WHERE rr.story_id = ss.id AND rr.alumni_id = %s) ) as viewer_reacted"
            params.append(viewer_alumni_id)
        ...
        for row in rows:
            ...
            # include reaction meta if available
            if 'reaction_count' in row:
                story['reaction_count'] = int(row.get('reaction_count') or 0)
            else:
                story['reaction_count'] = 0
            if 'comment_count' in row:
                story['comment_count'] = int(row.get('comment_count') or 0)
            else:
                story['comment_count'] = 0
            if 'viewer_reacted' in row:
                try:
                    story['viewer_reacted'] = bool(row.get('viewer_reacted'))
                except Exception:
                    story['viewer_reacted'] = False
            else:
                story['viewer_reacted'] = False
...
@app.route('/api/success-stories/<int:story_id>/react', methods=['POST', 'OPTIONS'])
def toggle_story_react(story_id):
    """Toggle a 'love' react for a story for the given alumni (alumni_id or student_id)."""
    try:
        if request.method == 'OPTIONS':
            return ('', 204)

        data = request.get_json(silent=True) or request.form or {}
        alumni_id_str = data.get('alumni_id') or request.args.get('alumni_id') or ''
        student_id_str = data.get('student_id') or request.args.get('student_id') or ''
        ...
        cur = conn.cursor()
        # verify story exists
        cur.execute('SELECT id FROM success_stories WHERE id=%s LIMIT 1', (story_id,))
        r = cur.fetchone()
        if not r:
            cur.close()
            return jsonify({'success': False, 'message': 'Story not found'}), 404
        ...
        cur.execute('SELECT id FROM story_reactions WHERE story_id=%s AND alumni_id=%s LIMIT 1', (story_id, resolved_alumni_id))
        existing = cur.fetchone()
        if existing:
            # remove reaction (toggle off)
            cur.execute('DELETE FROM story_reactions WHERE id=%s', (existing['id'] if isinstance(existing, dict) else existing[0],))
            conn.commit()
            reacted = False
        else:
            # add reaction
            cur.execute('INSERT INTO story_reactions (story_id, alumni_id, created_at) VALUES (%s, %s, NOW())', (story_id, resolved_alumni_id))
            conn.commit()
            reacted = True

        # get new count
        cur.execute('SELECT COUNT(*) as cnt FROM story_reactions WHERE story_id=%s', (story_id,))
        cnt_row = cur.fetchone()
        cur.close()
        count = cnt_row['cnt'] if isinstance(cnt_row, dict) else (cnt_row[0] if cnt_row else 0)

        return jsonify({'success': True, 'reacted': bool(reacted), 'count': int(count)})
    except Exception as e:
        print(f'[ERROR] toggle_story_react: {e}')
        return jsonify({'success': False, 'message': 'Failed to toggle reaction'}), 500


@app.route('/api/success-stories/<int:story_id>/reactions', methods=['GET'])
def get_story_reactions(story_id):
    try:
        ...
        r = cur.fetchone()
        cur.close()
        count = r['cnt'] if isinstance(r, dict) else (r[0] if r else 0)
        return jsonify({'success': True, 'count': int(count)})
    except Exception as e:
        print(f'[ERROR] get_story_reactions: {e}')
        return jsonify({'success': False, 'message': 'Failed to fetch reactions'}), 500


@app.route('/api/success-stories/<int:story_id>/comments', methods=['POST', 'OPTIONS'])
def post_story_comment(story_id):
    try:
        ...
        parent_id = data.get('parent_comment_id') or data.get('parent') or None
        alumni_id_str = data.get('alumni_id') or request.args.get('alumni_id') or ''
        student_id_str = data.get('student_id') or request.args.get('student_id') or ''
        ...
        if not comment_text:
            return jsonify({'success': False, 'message': 'comment_text is required'}), 400
        ...
        cur.execute('INSERT INTO story_comments (story_id, alumni_id, parent_comment_id, comment_text, created_at) VALUES (%s, %s, %s, %s, NOW())', (story_id, resolved_alumni_id, parent_id_int, comment_text))
        conn.commit()
        new_id = cur.lastrowid

        # fetch created comment
        cur.execute('SELECT id, story_id, alumni_id, parent_comment_id, comment_text, created_at FROM story_comments WHERE id=%s', (new_id,))
        ...
        import traceback
        print(f'[ERROR] post_story_comment: {e}')
        traceback.print_exc()
        return jsonify({'success': False, 'message': 'Failed to post comment'}), 500


@app.route('/api/success-stories/<int:story_id>/comments', methods=['GET'])
def get_story_comments(story_id):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute('''
            SELECT sc.id, sc.story_id, sc.alumni_id, sc.parent_comment_id, sc.comment_text, sc.created_at,
                   a.name as alumni_name, a.photo as alumni_photo
            FROM story_comments sc
            LEFT JOIN alumni a ON sc.alumni_id = a.id
            WHERE sc.story_id=%s
        ...
        for row in rows:
            comments.append({
                ...
                'alumni_id': row.get('alumni_id'),
                'alumni_name': row.get('alumni_name') or 'Anonymous',
                'alumni_photo': row.get('alumni_photo'),
        ...
        import traceback
        print(f'[ERROR] get_story_comments: {e}')
        traceback.print_exc()
...
def _run_migrations():
    ...
    try:
        # Reuse safe migration path that does not require Flask request/app context.
        ensure_db_migrations()
    except Exception as e:
        print(f'[MIGRATION] Error: {e}')


# Temporary debug endpoint to list routes
...
def _debug_routes():
    try:
        ...
        print(f'[DEBUG] Failed to list routes: {e}')
        return jsonify({'success': False, 'message': 'Failed to list routes'}), 500
