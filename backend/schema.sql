...
-- Admin accounts (super admin + co-admins)
...
-- Story reactions: one row per (story_id, alumni_id) representing a "love" react
CREATE TABLE IF NOT EXISTS story_reactions (
    id        INT AUTO_INCREMENT PRIMARY KEY,
    story_id  INT NOT NULL,
    alumni_id INT NOT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_story_reactor (story_id, alumni_id),
    FOREIGN KEY (story_id) REFERENCES success_stories(id) ON DELETE CASCADE,
    FOREIGN KEY (alumni_id) REFERENCES alumni(id) ON DELETE CASCADE,
    INDEX idx_story_reactions_story (story_id),
    INDEX idx_story_reactions_alumni (alumni_id)
);

-- Story comments: supports replies via parent_comment_id
