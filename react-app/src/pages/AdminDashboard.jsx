...
import { formatDateTimeInBDT } from '../utils/dateFormatUtils'
import {
  getPending, getAlumni, getStudents, approveAlumni, rejectAlumni, deleteAlumni,
  getEvents, addEvent, deleteEvent, updateEvent, uploadEventImage, uploadEventAttachment,
  getTransactions, addTransaction, deleteTransaction,
  getFundRequests, addFundRequest, updateFundRequest, deleteFundRequest,
...
// Small reusable Avatar component — renders either image or initial fallback (NO OVERLAP)
function Avatar({ src, name, size = 48, alt, onError }) {
  ...
  // If image failed or no src, show only fallback div
  if (!src || imageFailed) {
    return (
      <div className="table-avatar" style={containerStyle} title={name}>
        <span className="table-avatar-initial">{initial}</span>
      </div>
    );
  }
  
  // If image exists and hasn't failed, show only the image
  return (
    <div style={containerStyle}>
      <img
        ...
        onError={handleError}
        style={{width: size, height: size, minWidth: size, minHeight: size, maxWidth: size, maxHeight: size, display: 'block', objectFit: 'cover'}}
      />
    </div>
  );
}
...
const readAdminSession = () => {
  ...
  if (raw === 'true') {
    return { authenticated: true, role: 'super_admin', username: 'admin', token: null, legacy: true }
  }
...
export default function AdminDashboard() {
  ...
  const adminRole = adminSession?.role || (localStorage.getItem('adminSession') === 'true' ? 'super_admin' : '')
  const adminToken = adminSession?.token || ''
  const canAccessAdminTools = adminRole === 'super_admin'
  const adminBadgeLabel = adminRole === 'super_admin' ? 'Super Admin' : 'Co-admin'

  // Download attendees list as PDF
  const downloadAttendeesPDF = (event, attendeesList) => {
    if (!attendeesList || attendeesList.length === 0) return

    // Create PDF document with landscape orientation so the table has room to breathe
    const doc = new jsPDF({
    ...
    doc.setFontSize(16)
    doc.text(`Attendees List — ${event.title}`, 14, yPosition)
    yPosition += 10

    // Event details
    doc.setFont('Helvetica', 'normal')
    doc.setFontSize(10)
    const eventDate = event.date ? new Date(event.date + 'T00:00:00').toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'
    const eventTime = event.time || 'N/A'
    const eventLocation = event.location || 'N/A'
    doc.text(`Date: ${eventDate} | Time: ${eventTime} | Location: ${eventLocation}`, 14, yPosition)
    yPosition += 8

    // Table data
    const tableData = attendeesList.map((attendee, index) => [
      index + 1,
      attendee.name || '—',
      attendee.student_id || '—',
      attendee.session || '—',
    ...
    if (typeof doc.autoTable === 'function') {
      doc.autoTable({
        head: [['#', 'Name', 'Student ID', 'Session', 'Email', 'Phone']],
        body: tableData,
        startY: yPosition,
        tableWidth: 'auto',
      ...
      const colWidths = [10, 44, 28, 28, 62, 35]
      doc.setDrawColor(200)
      doc.setLineWidth(0.5)
      
      // Header row
      doc.setFillColor(15, 78, 168)
      doc.setTextColor(255, 255, 255)
      doc.setFont('Helvetica', 'bold')
      const headers = ['#', 'Name', 'Student ID', 'Session', 'Email', 'Phone']
      let xPos = 10
      headers.forEach((header, index) => {
        const width = colWidths[index]
        doc.rect(xPos, yPosition, width, 8, 'F')
        doc.text(header, xPos + 1, yPosition + 5.5, { maxWidth: width - 2 })
        xPos += width
      })
      yPosition += 8
      ...
      tableData.forEach((row, idx) => {
        ...
        row.forEach((cell, cellIdx) => {
          ...
          doc.text(String(cell), xPos + 1, yPosition + 5.3, { maxWidth: width - 2 })
          xPos += width
        })
        yPosition += rowHeight
      })
    ...
    doc.text(`Generated on ${currentDate}`, 10, pageHeight - 8)
  ...
  // Image error handler - show fallback avatar
  ...
  const [newEvent,       setNewEvent]       = useState({ title:'', date:'', time:'', registration_deadline:'', location:'', description:'', fee:'', payment_account:'', audience:'both', banner_image_file: null, attachment_file: null })
  ...
  const [showAttendeesEmailModal, setShowAttendeesEmailModal] = useState(false)
  const [attendeesEmailSubject, setAttendeesEmailSubject] = useState('')
  const [attendeesEmailPreheader, setAttendeesEmailPreheader] = useState('')
  const [attendeesEmailBody, setAttendeesEmailBody] = useState('')
  const [attendeesEmailCta, setAttendeesEmailCta] = useState('Visit AlumniConnect')
  const [attendeesEmailSending, setAttendeesEmailSending] = useState(false)
  const [selectedEventForModal, setSelectedEventForModal] = useState(null)
  const [showEventDetailsModal, setShowEventDetailsModal] = useState(false)
  const [showFundReqModal, setShowFundReqModal] = useState(false)
  const [editingFundReqId, setEditingFundReqId] = useState(null)
  const [newFundReq, setNewFundReq] = useState({
    title:'', purpose:'', target_amount:'', payment_option:'both', status:'open',
    bkash_number:'', bank_account_name:'', bank_account_number:'', bank_name:''
  })
  const [fundRequestSearch, setFundRequestSearch] = useState('')
  ...
  const [newAdminForm, setNewAdminForm] = useState({ name: '', username: '', password: '', confirmPassword: '' })
  const [adminCreating, setAdminCreating] = useState(false)
  const [adminDeleting, setAdminDeleting] = useState(null)
  const [deleteConfirmation, setDeleteConfirmation] = useState(null)
  const [adminNameUpdating, setAdminNameUpdating] = useState({})
  const [adminPasswordVisibility, setAdminPasswordVisibility] = useState({})
  const [adminKnownPasswords, setAdminKnownPasswords] = useState({})
  ...
  const sortedAdminAccounts = useMemo(() => {
    return [...adminAccounts].sort((a, b) => {
      const aRolePriority = a?.role === 'super_admin' ? 0 : 1
      const bRolePriority = b?.role === 'super_admin' ? 0 : 1
  ...
  const attendeeEmailRecipients = useMemo(() => {
    const seen = new Set()
  ...
  const attendeeEmailCount = attendeeEmailRecipients.length

  const resetAttendeesEmailDraft = useCallback(() => {
    setAttendeesEmailSubject('')
    setAttendeesEmailPreheader('')
    setAttendeesEmailBody('')
    setAttendeesEmailCta('Visit AlumniConnect')
    setAttendeesEmailSending(false)
  }, [])

  const openAttendeesEmailComposer = useCallback(() => {
    if (attendeeEmailRecipients.length === 0) {
      alert('No attendee email addresses were found for this event.')
      return
    }

    setAttendeesEmailSubject(attendeesEvent?.title ? `Update for ${attendeesEvent.title}` : '')
    setAttendeesEmailPreheader('')
    setAttendeesEmailBody('')
    setAttendeesEmailCta('Visit AlumniConnect')
    setShowAttendeesEmailModal(true)
  }, [attendeeEmailRecipients.length, attendeesEvent])

  const handleSendAttendeesEmail = async () => {
    if (!attendeeEmailRecipients.length) return

    const subject = attendeesEmailSubject.trim()
    const message = attendeesEmailBody.trim()
    if (!subject || !message) {
      alert('Subject and message are required.')
      return
    }

    setAttendeesEmailSending(true)
    try {
      const payload = {
        recipient_emails: attendeeEmailRecipients,
        subject,
        preheader: attendeesEmailPreheader.trim(),
        message,
        cta_text: attendeesEmailCta.trim() || 'Visit AlumniConnect',
      }
      const { ok, data } = await sendAdminEmail(payload)
      if (ok && data?.success) {
        alert(data?.message || `Email sent successfully to ${attendeeEmailRecipients.length} attendees. Sent: ${data?.sent || 0}, Failed: ${data?.failed || 0}`)
        setShowAttendeesEmailModal(false)
        resetAttendeesEmailDraft()
        return
      }

      alert(data?.message || 'Email send failed. Please try again.')
    } catch (error) {
      alert(error?.message || 'Email send failed. Please try again.')
    } finally {
      setAttendeesEmailSending(false)
    }
  }
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (pd.ok) setPending(normalizePeople(pd.data).filter(person => person.status === 'pending'))
  ...
  useEffect(() => {
    ...
    fetchAll()
  }, [adminSession, fetchAll, navigate])
  ...
  const fetchAdminAccounts = useCallback(async () => {
    ...
    try {
      const { ok, data } = await getAdminAccounts()
      if (ok) {
        setAdminAccounts(Array.isArray(data?.admins) ? data.admins : [])
      } else {
        setAdminAccountsError(data?.message || 'Could not load admin accounts.')
      }
    } catch (error) {
      setAdminAccountsError(error?.message || 'Could not load admin accounts.')
    } finally {
      setAdminAccountsLoading(false)
    }
  }, [canAccessAdminTools])

  const handleDeleteAdminAccount = async (adminId) => {
    setAdminDeleting(adminId)
    try {
      const { ok, data } = await deleteAdminAccount(adminId)
      if (ok) {
        alert(data?.message || 'Co-admin deleted successfully.')
        setDeleteConfirmation(null)
        await fetchAdminAccounts()
      } else {
        alert(data?.message || 'Could not delete co-admin.')
      }
    } catch (error) {
      alert(error?.message || 'Could not delete co-admin.')
    } finally {
      setAdminDeleting(null)
      setAdminPasswordVisibility((prev) => {
        if (!(adminId in prev)) return prev
        const next = { ...prev }
        delete next[adminId]
        return next
      })
      setAdminKnownPasswords((prev) => {
        if (!(adminId in prev)) return prev
        const next = { ...prev }
        delete next[adminId]
        return next
  ...
  const handleSetAdminName = (admin) => {
    const currentName = (admin?.name || '').trim()
    setAdminNameInput(currentName)
    setEditingAdminId(admin.id)
    setShowAdminNameModal(true)
  }

  const handleSaveAdminName = async () => {
    const nextName = adminNameInput.trim()
    if (nextName.length < 2) {
      alert('Co-admin name must be at least 2 characters.')
      return
    }

    setAdminNameUpdating((prev) => ({ ...prev, [editingAdminId]: true }))
    try {
      const { ok, data } = await updateAdminAccountName(editingAdminId, nextName)
      if (!ok) {
        alert(data?.message || 'Could not update co-admin name.')
        return
      }
      ...
      alert(error?.message || 'Could not update co-admin name.')
    } finally {
      setAdminNameUpdating((prev) => {
        const next = { ...prev }
        delete next[editingAdminId]
        return next
      })
    }
  }
  ...
  useEffect(() => {
    if (activeView === 'more-admin' && canAccessAdminTools) {
      fetchAdminAccounts()
    }
  }, [activeView, canAccessAdminTools, fetchAdminAccounts])

  useEffect(() => {
    if (activeView === 'more-admin' && !canAccessAdminTools) {
      setActiveView('dashboard')
    }
  }, [activeView, canAccessAdminTools])
  ...
  const handleNewEventAttachmentChange = (e) => {
    const file = e.target.files?.[0]
    setNewEvent({ ...newEvent, attachment_file: file || null })
  }
  ...
  const handleEditEventAttachmentChange = (e) => {
    const file = e.target.files?.[0]
    setEditingEvent({ ...editingEvent, attachment_file: file || null })
  }
  ...
  const handleAddEvent = async (e) => {
    ...
    formData.append('fee', newEvent.fee ? String(Number(newEvent.fee)) : '0')
    formData.append('payment_account', newEvent.payment_account || '')
    formData.append('audience', newEvent.audience || 'both')
    if (newEvent.banner_image_file) {
      formData.append('banner_image', newEvent.banner_image_file)
    }
    if (newEvent.attachment_file) {
      formData.append('attachment', newEvent.attachment_file)
    }

    const { ok, status, data } = await addEvent(formData)
    console.log('[handleAddEvent] Response:', { ok, status, data })
    if (ok) {
      ...
      const createdWithMedia = {
        ...createdEvent,
        banner_image_file: null,
        attachment: data?.attachment || data?.event?.attachment || null,
        attachment_url: data?.attachment_url || data?.event?.attachment_url || null,
      }
      setEvents(prev => [{ ...createdWithMedia }, ...prev])
      setStats(s => ({ ...s, events: s.events + 1 }))
      setNewEvent({ title:'', date:'', time:'', registration_deadline:'', location:'', description:'', fee:'', payment_account:'', audience:'both', banner_image_file: null, attachment_file: null })
      setShowModal(false)
    } else {
      alert(`Failed to create event: ${data?.message || `HTTP ${status}`}`)
    }
  }

  const handleUpdateEvent = async (e) => {
    e.preventDefault()
    const { ok } = await updateEvent(editingEvent.id, { title: editingEvent.title, date: editingEvent.date, time: editingEvent.time, registration_deadline: editingEvent.registration_deadline, location: editingEvent.location, description: editingEvent.description, fee: editingEvent.fee ? Number(editingEvent.fee) : 0, payment_account: editingEvent.payment_account, audience: editingEvent.audience })
    if (ok) {
      // Upload image if a new one is selected
      let bannerImage = editingEvent.banner_image || null
      let bannerImageUrl = editingEvent.banner_image_url || null
      if (editingEvent.banner_image_file) {
        const uploadResult = await uploadEventImage(editingEvent.id, editingEvent.banner_image_file)
        if (uploadResult.ok) {
          bannerImage = uploadResult.data?.banner_image || bannerImage
          bannerImageUrl = uploadResult.data?.banner_image_url || bannerImageUrl
        }
      }
      // Upload attachment if selected
      let attachment = editingEvent.attachment || null
      let attachmentUrl = editingEvent.attachment_url || null
      if (editingEvent.attachment_file) {
        const attachRes = await uploadEventAttachment(editingEvent.id, editingEvent.attachment_file)
        if (attachRes.ok) {
          attachment = attachRes.data?.attachment || attachment
          attachmentUrl = attachRes.data?.attachment_url || attachmentUrl
        }
      }
      const updatedEvent = { ...editingEvent, fee: Number(editingEvent.fee)||0, banner_image: bannerImage, banner_image_url: bannerImageUrl, banner_image_file: null, attachment: attachment, attachment_url: attachmentUrl, attachment_file: null }
      setEvents(prev => prev.map(ev => ev.id === editingEvent.id ? updatedEvent : ev))
      // If the details modal is open for this event, refresh it so attachment shows immediately
      if (selectedEventForModal && selectedEventForModal.id === editingEvent.id) {
  ...
  const handleAddFundRequest = async (e) => {
    ...
    const payload = {
      ...
      created_by: 'admin',
      status: newFundReq.status || 'open',
    }
    try {
      const response = editingFundReqId
        ? await updateFundRequest(editingFundReqId, payload)
        : await addFundRequest(payload)
      const { ok, data } = response
      if (!ok) {
        alert(editingFundReqId ? 'Could not update request. Please try again.' : 'Could not publish request. Please try again.')
        setShowFundReqModal(false)
        setActiveView('alumni')
        return
      }

      const savedRequest = { ...payload, id: data.id || editingFundReqId }
      setFundRequests(prev => {
        if (editingFundReqId) {
          return prev.map(req => req.id === editingFundReqId ? { ...req, ...savedRequest } : req)
        }
  ...
  const handleEditFundRequest = (fundRequest) => {
    setEditingFundReqId(fundRequest.id)
    setNewFundReq({
      title: fundRequest.title || '',
      purpose: fundRequest.purpose || '',
      target_amount: fundRequest.target_amount ?? '',
      payment_option: fundRequest.payment_option || 'both',
      status: fundRequest.status || 'open',
      bkash_number: fundRequest.bkash_number || '',
  ...
  const handleDeleteFundRequest = async (id) => {
    ...
    if (ok) {
      ...
      try {
        const { ok: frOk, data: frData } = await getFundRequests({ status: 'open' })
        if (frOk) setFundRequests(frData)
        else setFundRequests(prev => prev.filter(req => req.id !== id))
      } catch (_) {
        setFundRequests(prev => prev.filter(req => req.id !== id))
      }

      if (editingFundReqId === id) {
        setEditingFundReqId(null)
        setNewFundReq({
          title:'', purpose:'', target_amount:'', payment_option:'both', status:'open',
          bkash_number:'', bank_account_name:'', bank_account_number:'', bank_name:''
      ...
      alert('Failed to delete fund request. Please try again or check server logs.')
    }
  }

  const filteredTransactions = useMemo(() => {
    const query = fundRequestSearch.trim().toLowerCase()
  ...
  }, [transactions, fundRequestSearch])
  ...
  const handleSendEmail = async () => {
    ...
    if (ok && data?.success) {
      alert(data?.message || `Email sent successfully. Sent: ${data.sent}, Failed: ${data.failed}`)
      return
    }
  ...
  const navItems = [
    ...
    ...(canAccessAdminTools ? [{ view:'more-admin', icon:'fa-user-gear', label:'More Admin', section:'Administration' }] : []),
  ]
  const titles = {
    ...
    'more-admin':  { title:'More Admin',        sub:'Create and manage co-admin accounts' },
    ...
    <div className="admin-wrapper">
      ...
      <div className="admin-main">
        ...
        <div className="admin-topbar">
          ...
          <div style={{display:'flex', alignItems:'center', gap:12}}>
            <div className="admin-badge"><i className="fa-solid fa-user-shield"></i> {adminBadgeLabel}</div>
        ...
        <div className="admin-content">
          ...
          {activeView === 'dashboard' && (
            <>
              ...
              <PendingCards rows={pending.slice(0,3)} onApprove={handleApprove} onReject={handleReject} onImageError={handleImageError} />
            </>
          )}
                  ...
                  <table className="admin-table">
                    ...
                    <tbody>
                      {alumni.filter(a => {
                        ...
                        <tr key={a.id}>
                          <td>{i+1}</td>
                          <td><div className="alumni-name-cell"><Avatar src={a.photo_url} name={a.name} onError={handleImageError} />{a.name}</div></td>
                          <td>{a.student_id || '\u2014'}</td>
                          <td>{a.email}</td>
                          <td>{a.phone}</td>
                          <td>{a.session}</td>
                          ...
                          <td>
                            <span className="status-badge approved">
                              {a.approved_by && a.approved_at
                                ? `Approved by ${a.approved_by} on ${new Date(a.approved_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`
                                : 'Approved'}
                            </span>
                          </td>
          ...
          {/* MORE ADMIN */}
          {activeView === 'more-admin' && canAccessAdminTools && (
            <div style={{display:'grid', gap:20}}>
              {!adminToken && (
                <div style={{background:'#fff4db', border:'1px solid #f3cf84', color:'#7a4b00', borderRadius:16, padding:'14px 18px', fontWeight:600}}>
                  Legacy super-admin session detected. For creating co-admin accounts, logout and login again once so the new secure admin token is generated.
                </div>
              )}
              <div style={{background:'#fff', border:'1px solid #e6eef8', borderRadius:20, padding:24, boxShadow:'0 10px 30px rgba(15,78,168,0.06)'}}>
                <div className="section-title" style={{marginTop:0}}>
                  <i className="fa-solid fa-user-gear" style={{color:'#0f4ea8'}}></i>
                  Create Co-Admin
                </div>
                <p style={{marginTop:-8, marginBottom:18, color:'#5d6b82'}}>Only the super admin can create co-admin accounts. Co-admins can log in normally, but they will not see this menu.</p>

                <form onSubmit={async (e) => {
                  e.preventDefault()
                  setAdminAccountsError('')
                  const draftPassword = newAdminForm.password

                  if (newAdminForm.password !== newAdminForm.confirmPassword) {
                    setAdminAccountsError('Passwords do not match.')
                    return
                  }
                  ...
                  try {
                    ...
                    if (!ok) {
                      setAdminAccountsError(data?.message || 'Could not create co-admin.')
                      return
                    }

                    const createdId = data?.admin?.id
                    const temporaryPassword = data?.temporary_password || draftPassword
                    if (createdId && temporaryPassword) {
                      setAdminKnownPasswords((prev) => ({ ...prev, [createdId]: temporaryPassword }))
                    }
                    ...
                    setAdminAccountsError(error?.message || 'Could not create co-admin.')
                  } finally {
                    setAdminCreating(false)
                  }
                }} style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))', gap:16}}>
                  <div>
                    <label style={{display:'block', marginBottom:6, fontWeight:600}}>Co-Admin Name</label>
                    <input
                      type="text"
                      value={newAdminForm.name}
                      onChange={(e) => setNewAdminForm((prev) => ({ ...prev, name: e.target.value }))}
                      placeholder="Enter co-admin's full name"
                      required
                      style={{width:'100%', padding:'12px 14px', border:'1px solid #d8e4f5', borderRadius:12, outline:'none'}}
                    />
                  </div>
                  <div>
                  ...
                  <div style={{display:'flex', alignItems:'end'}}>
                    <button
                      ...
                      style={{width:'100%', border:'none', borderRadius:12, padding:'12px 16px', background:'#0f4ea8', color:'#fff', fontWeight:700, cursor:'pointer'}}
                    >
                      {adminCreating ? 'Creating…' : 'Create Co-Admin'}
                    </button>
                  </div>
                </form>
                {adminAccountsError && <div style={{marginTop:16, color:'#b42318', fontWeight:600}}>{adminAccountsError}</div>}
              </div>

              <div>
                <div className="section-title">
                  <i className="fa-solid fa-users-gear" style={{color:'#0f4ea8'}}></i>
                  Admin Accounts
                  <span className="badge-count">{sortedAdminAccounts.length}</span>
                </div>

                {adminAccountsLoading ? (
                  <div className="empty-state"><i className="fa-solid fa-spinner fa-spin"></i><p>Loading admin accounts…</p></div>
                ) : sortedAdminAccounts.length === 0 ? (
                  <div className="empty-state"><i className="fa-solid fa-user-lock"></i><p>No admin accounts found.</p></div>
                ) : (
                  <div className="admin-table-wrap">
                    <table className="admin-table">
                      <thead>
                      ...
                      <tbody>
                        {sortedAdminAccounts.map((admin, index) => {
                          const isVisible = Boolean(adminPasswordVisibility[admin.id])
                          const knownPassword = adminKnownPasswords[admin.id]
                          const displayPassword = knownPassword || 'Not available'
                          const coAdminDisplayName = (admin.name || '').trim() || ''
                          ...
                          <tr key={admin.id}>
                            ...
                            <td>
                              {coAdminDisplayName || (
                                <button
                                  type="button"
                                  onClick={() => handleSetAdminName(admin)}
                                  disabled={Boolean(adminNameUpdating[admin.id])}
                                  style={{
                                    ...
                                    fontSize: '12px',
                                    fontWeight: 600,
                                    cursor: adminNameUpdating[admin.id] ? 'not-allowed' : 'pointer',
                                    opacity: adminNameUpdating[admin.id] ? 0.65 : 1,
                                  }}
                                >
                                  {adminNameUpdating[admin.id] ? 'Saving…' : 'Set Name'}
                                </button>
                              )}
                            </td>
                            ...
                            <td>
                              <div className="admin-password-cell">
                                <span>{isVisible ? displayPassword : '••••••••'}</span>
                                <button
                                  type="button"
                                  className="admin-password-toggle"
                                  onClick={() => setAdminPasswordVisibility((prev) => ({ ...prev, [admin.id]: !prev[admin.id] }))}
                                  title={isVisible ? 'Hide password' : 'View password'}
                                  aria-label={isVisible ? 'Hide password' : 'View password'}
                                >
                                  <i className={`fa-solid ${isVisible ? 'fa-eye-slash' : 'fa-eye'}`}></i>
                                </button>
                              </div>
                            </td>
                            <td style={{textTransform:'capitalize'}}>{String(admin.role || '').replace('_', ' ')}</td>
                            <td>
                              <span className={`status-badge ${admin.is_active ? 'approved' : 'rejected'}`}>
                                {admin.is_active ? 'Active' : 'Disabled'}
                              </span>
                            </td>
                            <td>{admin.created_by || '—'}</td>
                            <td>{formatDateTimeInBDT(admin.last_login_at)}</td>
                            <td>
                              {admin.role === 'co_admin' && admin.is_active && (
                                <button
                                  onClick={() => setDeleteConfirmation(admin)}
                                  ...
                                  style={{
                                    ...
                                    fontSize: '12px',
                                    fontWeight: 600,
                                    cursor: adminDeleting === admin.id ? 'not-allowed' : 'pointer',
                                    opacity: adminDeleting === admin.id ? 0.6 : 1,
                                  }}
                                >
                                  {adminDeleting === admin.id ? 'Deleting…' : 'Delete'}
                                </button>
                              )}
                            </td>
                          </tr>
          ...
          {activeView === 'transactions' && (
            <>
                  ...
                  {fundRequests.map((r) => (
                    <div className="event-card" key={r.id} style={{borderTopColor:'#00a3a3'}}>
                      ...
                      <div className="event-actions">
                        <button type="button" className="btn-edit" onClick={() => handleEditFundRequest(r)}><i className="fa-solid fa-pen"></i> Edit</button>
                        <button type="button" className="btn-delete" onClick={() => handleDeleteFundRequest(r.id)}><i className="fa-solid fa-trash"></i> Delete</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{marginTop:24, marginBottom:12, display:'flex', gap:12, alignItems:'center', flexWrap:'wrap'}}>
                <div className="section-title" style={{margin:0}}>
                  <i className="fa-solid fa-magnifying-glass" style={{color:'#00a3a3'}}></i>
                  Donor Search
                </div>
                <input
                  type="text"
                  value={fundRequestSearch}
                  onChange={(e) => setFundRequestSearch(e.target.value)}
                  placeholder="Search by donor, request, type, amount, or transaction ID"
                  style={{
                    flex: '1 1 320px',
                    minWidth: 240,
                    padding: '12px 16px',
                    borderRadius: 12,
                    border: '1.5px solid #c5dbf5',
                    outline: 'none',
                    fontSize: 14,
                ...
                <div className="events-grid">
                  {events.map(ev => {
                    ...
                    <div className="event-card" key={ev.id}>
                      <div className="ec-banner admin-event-banner">
                        <BannerImage ev={ev} />
                      </div>
                      ...
                      {ev.registration_deadline && <p><i className="fa-solid fa-calendar-xmark"></i> Registration Deadline: {ev.registration_deadline}</p>}
                      ...
                      {/* Document link moved to Details modal */}
                      <p style={{marginTop:4}}>
                      ...
                      <div className="event-actions">
                        <button className="btn-details" onClick={() => { setSelectedEventForModal(ev); setShowEventDetailsModal(true); }} style={{background:'#fff',border:'1px solid #c5c5c5',marginRight:8}}>
      ...
      <Modal show={showAttendeesModal} onHide={() => { setShowAttendeesModal(false); setAttendeesSearch('') }} centered size="lg" scrollable contentClassName="bg-transparent border-0 shadow-none p-0">
        <div className="modal-box" style={{maxHeight:'75vh',overflowY:'auto'}}>
            ...
            <div className="modal-actions" style={{marginTop:20, display: 'flex', gap: '12px', justifyContent: 'flex-end'}}>
              <button
                type="button"
                className="btn-primary-admin"
                onClick={openAttendeesEmailComposer}
                disabled={attendeesLoading || attendeeEmailCount === 0}
                style={{padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '8px'}}
              >
                <i className="fa-solid fa-envelope"></i> Email to Attendees ({attendeeEmailCount})
              </button>
              <button 
                type="button" 
                className="btn-download"
                onClick={() => downloadAttendeesPDF(attendeesEvent, attendees.filter(a => {
                  const q = attendeesSearch.trim().toLowerCase()
                  if (!q) return true
                  return (a.name || '').toLowerCase().includes(q)
                      || (a.student_id || '').toLowerCase().includes(q)
                ...
                style={{padding: '10px 20px', backgroundColor: '#00a3a3', color: 'white', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px'}}
              >
      ...
      {attendeesEvent && (
      <Modal show={showAttendeesEmailModal} onHide={() => { setShowAttendeesEmailModal(false); resetAttendeesEmailDraft() }} centered size="lg" scrollable contentClassName="bg-transparent border-0 shadow-none p-0">
        <div className="modal-box" style={{maxHeight:'75vh',overflowY:'auto'}}>
          <h3 style={{marginBottom:4}}>
            <i className="fa-solid fa-envelope-open-text" style={{color:'#00a3a3',marginRight:8}}></i>
            Email to Attendees — {attendeesEvent.title}
          </h3>
          <p style={{fontSize:13,color:'#888',marginBottom:16}}>
            Send a custom email to all attendee emails for this event.
          </p>

          <div style={{display:'grid',gap:12,marginBottom:16}}>
            <div>
              <label style={{display:'block',fontSize:13,fontWeight:600,marginBottom:6}}>Recipients</label>
              <div style={{padding:'10px 12px',border:'1.5px solid #c5dbf5',borderRadius:10,background:'#f9fcff',fontSize:13,color:'#333'}}>
                {attendeeEmailCount} attendee email{attendeeEmailCount === 1 ? '' : 's'} selected
              </div>
            </div>
            <div>
              <label style={{display:'block',fontSize:13,fontWeight:600,marginBottom:6}}>Recipient preview</label>
              <div style={{padding:'10px 12px',border:'1.5px solid #e5eef7',borderRadius:10,background:'#fff',fontSize:13,color:'#555'}}>
                {attendeeEmailRecipients.slice(0, 5).join(', ')}{attendeeEmailRecipients.length > 5 ? ` +${attendeeEmailRecipients.length - 5} more` : ''}
              </div>
            </div>
            <div>
              ...
              <input
                type="text"
                value={attendeesEmailSubject}
                onChange={(e) => setAttendeesEmailSubject(e.target.value)}
                placeholder={`Update for ${attendeesEvent.title}`}
                style={{width:'100%',padding:'11px 14px',border:'1.5px solid #c5dbf5',borderRadius:10,fontSize:14,outline:'none',fontFamily:'Inter,sans-serif'}}
              />
            </div>
            <div>
              <label style={{display:'block',fontSize:13,fontWeight:600,marginBottom:6}}>Preheader (optional)</label>
              <input
                type="text"
                value={attendeesEmailPreheader}
                onChange={(e) => setAttendeesEmailPreheader(e.target.value)}
            ...
            <div>
              ...
              <textarea
                rows={8}
                value={attendeesEmailBody}
                onChange={(e) => setAttendeesEmailBody(e.target.value)}
                placeholder={`Write your message for attendees of ${attendeesEvent.title}...`}
                style={{width:'100%',padding:'11px 14px',border:'1.5px solid #c5dbf5',borderRadius:10,fontSize:14,outline:'none',fontFamily:'Inter,sans-serif',resize:'vertical'}}
              />
            </div>
            <div>
              <label style={{display:'block',fontSize:13,fontWeight:600,marginBottom:6}}>CTA button text</label>
              <input
                type="text"
                value={attendeesEmailCta}
                onChange={(e) => setAttendeesEmailCta(e.target.value)}
                placeholder="Visit AlumniConnect"
                style={{width:'100%',padding:'11px 14px',border:'1.5px solid #c5dbf5',borderRadius:10,fontSize:14,outline:'none',fontFamily:'Inter,sans-serif'}}
              />
            </div>
          ...
          <div className="modal-actions" style={{marginTop:20, display: 'flex', gap: '12px', justifyContent: 'flex-end'}}>
            <button type="button" className="btn-primary-admin" onClick={handleSendAttendeesEmail} disabled={attendeesEmailSending || attendeeEmailCount === 0 || !attendeesEmailSubject.trim() || !attendeesEmailBody.trim()} style={{padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '8px'}}>
              <i className={`fa-solid ${attendeesEmailSending ? 'fa-spinner fa-spin' : 'fa-paper-plane'}`}></i>
              {attendeesEmailSending ? 'Sending...' : `Send to ${attendeeEmailCount} Attendees`}
            </button>
            <button type="button" className="btn-cancel" onClick={() => { setShowAttendeesEmailModal(false); resetAttendeesEmailDraft() }}>Cancel</button>
          </div>
        </div>
      </Modal>
      )}
                ...
                <table className="admin-table">
                  ...
                  <tbody>
                    {trainAttendees.filter(a => {
                      ...
                      <tr key={a.id}>
                        <td>{i+1}</td>
                        <td><div className="alumni-name-cell"><Avatar src={a.photo_url} name={a.name} />{a.name}</div></td>
                        <td>{a.student_id || '—'}</td>
                        <td>{a.email || '—'}</td>
                        <td>{a.phone || '—'}</td>
      ...
      <Modal show={showModal} onHide={() => { 
        ...
        setNewEventPreview(null)
        setShowModal(false) 
      }} centered contentClassName="bg-transparent border-0 shadow-none p-0">
        <div className="modal-box">
            ...
            <form onSubmit={handleAddEvent}>
              ...
              <input type="date" value={newEvent.registration_deadline} onChange={e=>setNewEvent({...newEvent,registration_deadline:e.target.value})}/>
              <label>Time</label>
              ...
              {newEventPreview && (
                <div style={{marginTop: 8, borderRadius: 8, overflow: 'hidden', maxHeight: 150}}>
                  <img src={newEventPreview} alt="Banner preview" style={{width: '100%', height: 'auto', objectFit: 'cover'}} />
                </div>
              )}
              <label>Attachment (optional) — PDF or image</label>
              <input type="file" accept="image/*,application/pdf" onChange={handleNewEventAttachmentChange} />
      ...
      <Modal show={showEditModal} onHide={() => { 
        ...
        <div className="modal-box">
            ...
            <form onSubmit={handleUpdateEvent}>
              ...
              <label>Registration Deadline</label>
              <input type="date" value={editingEvent.registration_deadline||''} onChange={e=>setEditingEvent({...editingEvent,registration_deadline:e.target.value})}/>
              <label>Time</label>
              ...
              {(editingEventPreview || editingEvent.banner_image_url || editingEvent.banner_image) && (
                <div style={{marginTop: 8, borderRadius: 8, overflow: 'hidden', maxHeight: 150}}>
                  <img src={editingEventPreview || editingEvent.banner_image_url || getUploadUrl(editingEvent.banner_image)} alt="Banner preview" style={{width: '100%', height: 'auto', objectFit: 'cover'}} />
                </div>
              )}
              <label>Attachment (optional) — PDF or image</label>
              <input type="file" accept="image/*,application/pdf" onChange={handleEditEventAttachmentChange} />
              <label>Attachment (optional) — PDF or image</label>
              <input type="file" accept="image/*,application/pdf" onChange={handleEditEventAttachmentChange} />
              {(editingEvent.attachment_url || editingEvent.attachment) && (
                <div style={{marginTop:8}}>
                  <a href={editingEvent.attachment_url || getUploadUrl(editingEvent.attachment)} target="_blank" rel="noreferrer">Download existing attachment</a>
                </div>
              )}
              <label>Registration Fee (৳) — leave 0 if free</label>
      ...
      <Modal show={showFundReqModal} onHide={() => { setShowFundReqModal(false); setEditingFundReqId(null) }} centered contentClassName="bg-transparent border-0 shadow-none p-0">
        <div className="modal-box">
            <h3><i className="fa-solid fa-file-circle-plus" style={{color:'#00a3a3'}}></i> {editingFundReqId ? 'Edit Fund Request' : 'Create Fund Request'}</h3>
            <form onSubmit={handleAddFundRequest}>
              ...
              <label>Status</label>
              <select style={{padding:'11px 14px',border:'1.5px solid #c5dbf5',borderRadius:10,fontSize:14,outline:'none',fontFamily:'Inter,sans-serif'}} value={newFundReq.status || 'open'} onChange={e=>setNewFundReq({...newFundReq,status:e.target.value})}>
              ...
              <div className="modal-actions">
                <button type="submit" className="btn-primary-admin"><i className={`fa-solid ${editingFundReqId ? 'fa-save' : 'fa-plus'}`}></i> {editingFundReqId ? 'Save Changes' : 'Publish Request'}</button>
                <button type="button" className="btn-cancel" onClick={()=>{ setShowFundReqModal(false); setEditingFundReqId(null); setNewFundReq({ title:'', purpose:'', target_amount:'', payment_option:'both', status:'open', bkash_number:'', bank_account_name:'', bank_account_number:'', bank_name:'' }) }}>Cancel</button>
              </div>
            </form>
        </div>
      </Modal>
      ...
      {/* ADMIN NAME INPUT MODAL */}
      <Modal show={showAdminNameModal} onHide={() => setShowAdminNameModal(false)} centered contentClassName="bg-transparent border-0 shadow-none p-0">
        <div className="modal-box" style={{maxWidth: '420px'}}>
          <h3 style={{marginBottom: 16, color: '#0f4ea8'}}>
            <i className="fa-solid fa-user-edit" style={{marginRight: 8}}></i>
            Set Co-Admin Name
          </h3>
          ...
          <div className="modal-actions" style={{display: 'flex', gap: 10, justifyContent: 'flex-end'}}>
            <button
              ...
              onClick={() => setShowAdminNameModal(false)}
              disabled={adminNameUpdating[editingAdminId]}
              style={{
                padding: '8px 16px',
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '6px',
                fontWeight: 600,
                cursor: adminNameUpdating[editingAdminId] ? 'not-allowed' : 'pointer',
                opacity: adminNameUpdating[editingAdminId] ? 0.6 : 1,
              }}
            >
              {adminNameUpdating[editingAdminId] ? 'Saving...' : 'Save Name'}
            </button>
          </div>
        </div>
      </Modal>

      {/* DELETE CO-ADMIN CONFIRMATION MODAL */}
      {deleteConfirmation && (
        <Modal show={deleteConfirmation} onHide={() => setDeleteConfirmation(null)} centered contentClassName="bg-transparent border-0 shadow-none p-0">
          <div className="modal-box" style={{maxWidth: '420px'}}>
            <h3 style={{marginBottom: 12, color: '#dc3545'}}>
              <i className="fa-solid fa-exclamation-triangle" style={{marginRight: 8}}></i>
            ...
            <p style={{fontSize: 13, color: '#888', marginBottom: 20}}>
              This co-admin will be marked as deleted and will not be able to login anymore.
            </p>
            <div className="modal-actions" style={{display: 'flex', gap: 10, justifyContent: 'flex-end'}}>
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => handleDeleteAdminAccount(deleteConfirmation.id)}
                disabled={adminDeleting !== null}
                style={{
                  padding: '8px 16px',
                  ...
                  borderRadius: '6px',
                  fontWeight: 600,
                  cursor: adminDeleting !== null ? 'not-allowed' : 'pointer',
                  opacity: adminDeleting !== null ? 0.6 : 1,
                }}
              >
                {adminDeleting === deleteConfirmation.id ? 'Deleting...' : 'Delete Co-Admin'}
              </button>
            </div>
          </div>
        </Modal>
...
function PendingSection({ pending, upgradeRequests = [], pendingReferrals = [], pendingTab, setPendingTab, searchQuery, searchField, onApprove, onReject, onApproveUpgrade, onRejectUpgrade, onApproveReferral, onRejectReferral, onImageError }) {
    ...
    <>
      ...
      {pendingTab === 'upgrade'
        ...
        : pendingTab === 'referral'
          ...
          : <PendingCards rows={rows} onApprove={onApprove} onReject={onReject} onImageError={onImageError} />
      }
    </>
  )
}
...
function PendingCards({ rows, onApprove, onReject, onImageError }) {
  const [lightbox, setLightbox] = useState(null)
  
  const formatStatusBadge = (card) => {
    if (card.status === 'pending') {
      return '⏳ Pending Review'
    } else if (card.status === 'approved' && card.approved_by && card.approved_at) {
      const dateStr = new Date(card.approved_at).toLocaleDateString('en-GB', {
        day: '2-digit',
        month: 'short',
        year: 'numeric'
      })
      return `✓ Approved by ${card.approved_by} on ${dateStr}`
    } else if (card.status === 'rejected' && card.approved_by && card.approved_at) {
      const dateStr = new Date(card.approved_at).toLocaleDateString('en-GB', {
        day: '2-digit',
        month: 'short',
        year: 'numeric'
      })
      return `✗ Rejected by ${card.approved_by} on ${dateStr}`
    }
    return '⏳ Pending Review'
  }
        ...
        {rows.map((p) => (
          <div key={p.id} style={{
            ...
            <div style={{background:'linear-gradient(135deg,#0f4ea8,#00a3a3)', padding:'18px 22px', display:'flex', alignItems:'center', gap:16}}>
              {p.photo_url
                ? <img src={p.photo_url} alt={p.name} onError={onImageError} onClick={() => setLightbox(p.photo_url)}
            ...
            <div style={{padding:'16px 22px 6px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px 16px'}}>
              {[
                ...
                ['fa-home',            'Hall / College',  p.hall_name],
            ...
            <div style={{padding:'10px 22px 14px', display:'flex', gap:14}}>
              <div style={{flex:1}}>
                ...
                {p.photo_url
                  ? <img src={p.photo_url} alt="Profile" onError={onImageError} onClick={() => setLightbox(p.photo_url)}
              ...
              <div style={{flex:1}}>
                ...
                {p.id_photo_url
                  ? <img src={p.id_photo_url} alt="ID Card" onError={onImageError} onClick={() => setLightbox(p.id_photo_url)}
                      style={{width:'100%', height:120, objectFit:'cover', borderRadius:10, cursor:'zoom-in',
                        border:'2px solid #ece4f8', boxShadow:'0 2px 8px rgba(95,44,130,0.08)'}} />
                  : <div style={{width:'100%', height:120, borderRadius:10, background:'#edf6ff',
                      display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:6,
            ...
            {p.status === 'pending' ? (
              ...
              <div style={{display:'flex', gap:10, padding:'0 22px 18px'}}>
                <div style={{flex:1, padding:'8px 12px', borderRadius:8, background:'#f5f5f5', color:'#666', textAlign:'center', fontSize:12, fontWeight:500}}>
                  {p.status === 'approved' ? '✓ Already Approved' : '✗ Already Rejected'}
                </div>
              </div>
            )}
          </div>
