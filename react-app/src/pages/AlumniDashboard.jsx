...
import { getStoredUser, normalizeProfileResponse } from '../utils/profile-utils'
...
const sidebarItems = [
  ...
  { view: 'success-story', icon: 'fa-star',           label: 'Success Story' },
...
const titles = {
  ...
  'success-story': { title: 'Success Story',   sub: 'Inspire and share your journey'              },
...
export default function AlumniDashboard() {
  ...
  const formatDateBadge = (dateStr) => {
    ...
    return parsed.toLocaleDateString('en-US', { day: '2-digit', month: 'short' }).replace(' ', ' ')
  }

  const formatDateFull = (dateStr) => {
    const parsed = parseDateValue(dateStr)
    if (!parsed) return String(dateStr || '')
    return parsed.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' }).replace(' ', ' ')
  }
  ...
  const formatTime12Hour = (timeStr) => {
    ...
    const direct = new Date(text)
    if (!Number.isNaN(direct.getTime()) && /[T\s]/.test(text)) {
      return direct.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
    }

    const parsedText = new Date(text)
    if (!Number.isNaN(parsedText.getTime())) {
      return parsedText.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
    }

    return text
  }

  const formatEventDateLine = (ev) => formatTime12Hour(ev?.event_time || ev?.time) || formatDateFull(ev?.date)
  ...
  const [addTrainForm,       setAddTrainForm]       = useState({ title:'', trainer:'', date:'', time:'', seats:'', status:'Upcoming', fee:'', payment_account:'' })
  
    const isTrainingExpired = (training) => {
      if (!training?.date) return false
      if (String(training.status || '').toLowerCase() !== 'upcoming') return false
  ...
  const [allStudents, setAllStudents] = useState([])
    const [alumniSearch, setAlumniSearch] = useSearchPreservation('alumni-dashboard-alumni-search', '')
  const [dashEventSearch, setDashEventSearch] = useState('')
  const [eventViewSearch, setEventViewSearch] = useState('')
  ...
  // Fetch profile from backend on mount to populate hall_name
  useEffect(() => {
    const fetchProfile = async () => {
      const currentUser = getStoredUser() || alumniInfo
      if (!currentUser?.id || !currentUser?.user_type) return

      try {
        const result = await getMyProfile(currentUser.id, currentUser.user_type)
        if (result?.data) {
          const fetchedProfile = normalizeProfileResponse(result.data.profile || result.data)
          setProfile(fetchedProfile)
          setEditData(fetchedProfile)
          try {
            localStorage.setItem('alumniUser', JSON.stringify(fetchedProfile))
          } catch (_) {}
        }
      } catch (error) {
  ...
  useEffect(() => {
    // Handle redirect from event details with activeView state
    if (location.state?.activeView) {
      setActiveView(location.state.activeView)
    }
    // Restore any saved dashboard event search from location state
    if (location.state?.dashEventSearch !== undefined) {
      setDashEventSearch(location.state.dashEventSearch)
    }
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem('alumni-force-dashboard-once') === '1') {
        setActiveView('dashboard')
        sessionStorage.removeItem('alumni-force-dashboard-once')
      }
    } catch {}
  }, [])
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('alumni-dashboard-active-view', activeView)
    } catch {
  ...
  useEffect(() => {
    ...
    const dashboardPath = basename + '/alumni-dashboard'
  ...
  const handleSaveProfile = async () => {
    ...
    setEditMode(false)
    if (alumniInfo.id) {
      const result = await updateAlumni(alumniInfo.id, {
        ...
        hall_name:             nextProfile.hall_name,
        bio:                   nextProfile.bio,
      ...
      if (result.ok) {
        const updatedProfile = normalizeProfileResponse(result.data.profile || result.data)
        setProfile(updatedProfile)
        setEditData(updatedProfile)
        localStorage.setItem('alumniUser', JSON.stringify(updatedProfile))
      }
    }
  }
  ...
  const handleAlumniAddTraining = async (e) => {
    ...
    try {
      ...
      if (ok) {
        ...
        setAddTrainForm({ title:'', trainer:'', date:'', time:'', seats:'', status:'Upcoming', fee:'', payment_account:'' })
  ...
  const openMyTrainAttendees = async (tr) => {
    ...
    const { ok, data } = await getTrainingAttendees(tr.id, alumniInfo.id)
    if (ok) setMyTrainAttendees(data)
    setMyTrainAttendeesLoading(false)
  }

  const isTrainingCreator = (training) => {
    const creatorId =
      training?.created_by ??
      training?.created_by_id ??
      training?.created_by_alumni_id

    if (!creatorId || !alumniInfo?.id) return false
    return String(creatorId) === String(alumniInfo.id)
  }
    ...
    <div className="ad-wrapper">
      ...
      <aside className="ad-sidebar">
        ...
        <div className="ad-alumni-card">
          <div className="ad-avatar-big">{getInitial(profile?.name)}</div>
      ...
      <div className="ad-main">
        ...
        <div className="ad-topbar">
          ...
          <div className="ad-topbar-right">
            ...
            {(profileAvatarUrl && !avatarLoadFailed)
              ? <img src={profileAvatarUrl} alt={profile.name} className="ad-topbar-profile-image" style={{cursor:'zoom-in'}} onError={() => setAvatarLoadFailed(true)} onClick={() => openImagePreview(profileAvatarUrl, profile.name)} />
              : <div className="ad-topbar-avatar">{getInitial(profile?.name)}</div>
            }
        ...
        <div className="ad-content" data-scroll-container="alumni-dashboard-content">
          ...
          {activeView === 'dashboard' && (
            <>
              ...
              <div className="ad-profile-card">
                {(profileAvatarUrl && !avatarLoadFailed)
                  ? <img src={profileAvatarUrl} alt={profile.name} className="ad-profile-image" style={{cursor:'zoom-in'}} onError={() => setAvatarLoadFailed(true)} onClick={() => openImagePreview(profileAvatarUrl, profile.name)} />
                  : <div className="ad-profile-avatar">{getInitial(profile?.name)}</div>
                }
                <div className="ad-profile-details">
                  ...
                  <div className="ad-profile-row"><span>Hall / Residential College</span><strong>{profile.hall_name || '—'}</strong></div>
                </div>
              </div>
              ...
              <div className="ad-section-title" style={{justifyContent:'space-between',flexWrap:'wrap'}}>
                <span style={{display:'flex',alignItems:'center',gap:10}}>
                  <i className="fa-solid fa-calendar-days" style={{color:'#00a3a3'}}></i> Events
              ...
              <div className="ad-events-grid">
                {events.filter(ev => ev.audience !== 'students' && (!dashEventSearch.trim() || ev.title.toLowerCase().includes(dashEventSearch.trim().toLowerCase()))).slice().sort((a, b) => b.id - a.id).slice(0, dashEventSearch.trim() ? undefined : 2).map(ev => {
                  ...
                  const attachmentHref = ev.attachment_url || (ev.attachment ? getUploadUrl(ev.attachment) : '')
                  return (
                  <div key={ev.id} className="ad-event-card">
                    {bannerUrl && (
                      <div className="ad-event-banner">
                        {ev.date && <div className="ad-event-date-badge">{formatDateBadge(ev.date)}</div>}
                        <img src={bannerUrl} alt={ev.title} />
                      </div>
                    )}
                    <div className="ad-event-content">
                      <div className="ad-event-date">
                        <i className="fa-solid fa-calendar"></i> {formatDateFull(ev.date)}
                        {(ev.event_time || ev.time || ev.date) && <div className="ad-event-time"><i className="fa-solid fa-clock"></i> {formatEventDateLine(ev)}</div>}
                      </div>
                      <h4>{ev.title}</h4>
                      ...
                      <div style={{display:'flex', gap:6, marginTop:8}}>
                        ...
                        <button
                          ...
                          onClick={() => { setSelectedEventForModal(ev); setShowEventDetailsModal(true); }}
                          style={{flex:1}}
                        >
                          Details
                        </button>
              ...
              <div style={{background:'white',borderRadius:18,overflow:'hidden',boxShadow:'0 4px 28px rgba(95,44,130,0.10)',border:'1px solid #ede8f8'}}>
                ...
                {filtered.map((a, idx) => (
                  <div key={a.id}
                    ...
                    onClick={() => {
                      try {
                        setSelectedProfileForModal({ ...a, past_jobs: getAlumniPastJobs(a) })
                  ...
                  >
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:13,minWidth:0,paddingRight:isCompactDirectory ? 0 : 12,marginBottom:isCompactDirectory ? 10 : 0}}>
                      <div style={{
                        ...
                        {resolveAvatarUrl(a)
                          ? <img src={resolveAvatarUrl(a)} alt={a.name} style={{width:'100%',height:'100%',objectFit:'cover',cursor:'zoom-in',display:'block'}} onClick={(e) => { e.stopPropagation(); openImagePreview(resolveAvatarUrl(a), a.name) }} />
                          : <span>{getInitial(a?.name)}</span>
                        }
                      </div>
                    ...
                    <div style={{display:'flex',flexDirection:'column',alignItems:isCompactDirectory ? 'flex-start' : 'center',gap:7}}>
                      ...
                      }} onClick={(e) => { e.stopPropagation(); setSelectedProfileForModal({ ...a, past_jobs: getAlumniPastJobs(a) }); setShowProfileCardModal(true); }}>
          ...
          {activeView === 'profile' && (
            <>
              {!editMode ? (
                <div style={{maxWidth:'700px',margin:'0 auto',background:'white',borderRadius:'24px',overflow:'hidden',boxShadow:'rgba(95, 44, 130, 0.25) 0px 24px 64px',position:'relative'}}>
                  <div style={{background:'linear-gradient(135deg, rgb(15, 78, 168), rgb(0, 163, 163))',borderRadius:'24px 24px 0px 0px',padding:'20px 24px 32px',position:'relative'}}>
                    <button style={{position:'absolute',top:'12px',right:'12px',background:'rgba(255, 255, 255, 0.2)',border:'none',borderRadius:'50%',width:'30px',height:'30px',cursor:'pointer',color:'white',fontSize:'15px',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={() => {}} title="Close">✕</button>
                    <h2 style={{color:'white',fontSize:'18px',fontWeight:800,margin:'0px'}}>{profile.name}</h2>
                    <p style={{color:'rgba(255, 255, 255, 0.8)',marginTop:'3px',fontSize:'13px'}}>{profile.student_id || 'N/A'} · {profile.department}</p>
                  </div>
                  <div style={{display:'flex',justifyContent:'center',position:'relative',zIndex:2}}>
                    <div style={{width:'96px',height:'96px',borderRadius:'50%',border:'4px solid white',background:'linear-gradient(135deg, rgb(0, 163, 163), rgb(15, 78, 168))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'36px',fontWeight:800,color:'white',marginTop:'-48px',overflow:'hidden',boxShadow:'rgba(95, 44, 130, 0.25) 0px 6px 24px'}}>
                      {(profileAvatarUrl && !avatarLoadFailed)
                        ? <img src={profileAvatarUrl} alt={profile.name} style={{width:'100%',height:'100%',objectFit:'cover',cursor:'zoom-in',display:'block'}} onError={() => setAvatarLoadFailed(true)} onClick={() => openImagePreview(profileAvatarUrl, profile.name)} />
                  ...
                  <div style={{padding:'20px 32px 32px'}}>
                    <div style={{textAlign:'center',marginBottom:'20px'}}>
                      <span style={{background:profile.status==='approved'?'rgb(232, 248, 240)':'rgb(255, 243, 205)',color:profile.status==='approved'?'rgb(26, 122, 74)':'rgb(153, 102, 0)',borderRadius:'20px',padding:'4px 16px',fontSize:'13px',fontWeight:700}}>
                        {profile.status === 'approved' ? '✔ Active Member' : '⏳ Pending Approval'}
                      </span>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderBottom:'1px solid rgb(243, 238, 255)'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-graduation-cap" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Department / Session</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.department} · {profile.session}</div></div>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderBottom:'1px solid rgb(243, 238, 255)'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-id-badge" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Student ID</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.student_id || 'N/A'}</div></div>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderBottom:'1px solid rgb(243, 238, 255)'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-envelope" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Email</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.email}</div></div>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderBottom:'1px solid rgb(243, 238, 255)'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-phone" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Phone</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.phone || 'N/A'}</div></div>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderBottom:'1px solid rgb(243, 238, 255)'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-home" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Hall / Residential</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.hall_name || 'N/A'}</div></div>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderBottom:'1px solid rgb(243, 238, 255)'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-building" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Organization</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.company || 'N/A'}</div></div>
                    </div>

                    <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px'}}>
                      <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-briefcase" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                      <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Designation</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.designation || 'N/A'}</div></div>
                    </div>

                    {profile.address && (
                      <div style={{display:'flex',alignItems:'flex-start',gap:'14px',padding:'10px 0px',borderTop:'1px solid rgb(243, 238, 255)',marginTop:'10px'}}>
                        <span style={{width:'36px',height:'36px',borderRadius:'10px',background:'rgb(243, 238, 255)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'1px'}}><i className="fa-solid fa-location-dot" style={{color:'rgb(15, 78, 168)',fontSize:'14px'}}></i></span>
                        <div><div style={{fontSize:'11px',color:'rgb(153, 153, 153)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.4px'}}>Address</div><div style={{fontSize:'14px',color:'rgb(51, 51, 51)',fontWeight:500,marginTop:'2px'}}>{profile.address}</div></div>
                      </div>
                    )}

                    <div style={{marginTop:'24px',display:'flex',gap:'12px'}}>
                      <button style={{flex:1,padding:'12px 20px',background:'linear-gradient(135deg, rgb(15, 78, 168), rgb(0, 163, 163))',color:'white',border:'none',borderRadius:'12px',fontSize:'14px',fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px'}} onClick={() => { setEditData(profile); setEditMode(true) }}>
                  ...
                  <div style={{maxWidth:'700px',margin:'0 auto',background:'white',borderRadius:'24px',overflow:'hidden',boxShadow:'rgba(95, 44, 130, 0.25) 0px 24px 64px',position:'relative'}}>
                    <div style={{background:'linear-gradient(135deg, rgb(15, 78, 168), rgb(0, 163, 163))',borderRadius:'24px 24px 0px 0px',padding:'20px 24px 32px',position:'relative'}}>
                      <h2 style={{color:'white',fontSize:'18px',fontWeight:800,margin:'0px'}}>Edit Profile</h2>
                      <p style={{color:'rgba(255, 255, 255, 0.8)',marginTop:'3px',fontSize:'13px'}}>Update your personal information</p>
                    </div>

                    <div style={{display:'flex',justifyContent:'center',position:'relative',zIndex:2}}>
                      <div style={{width:'96px',height:'96px',borderRadius:'50%',border:'4px solid white',background:'linear-gradient(135deg, rgb(0, 163, 163), rgb(15, 78, 168))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'36px',fontWeight:800,color:'white',marginTop:'-48px',overflow:'hidden',boxShadow:'rgba(95, 44, 130, 0.25) 0px 6px 24px'}}>
                        {(profileAvatarUrl && !avatarLoadFailed)
                          ? <img src={profileAvatarUrl} alt={profile.name} style={{width:'100%',height:'100%',objectFit:'cover',cursor:'pointer',display:'block'}} onError={() => setAvatarLoadFailed(true)} />
                          : getInitial(profile?.name)
                        }
                      </div>
                    </div>
                    ...
                    <div style={{padding:'20px 32px 32px'}}>
                      <div className="ad-form-row" style={{marginTop:'24px'}}>
                        <label style={{fontSize:'13px',fontWeight:600,color:'rgb(15, 78, 168)',textTransform:'uppercase',letterSpacing:'0.4px'}}>
                          <i className="fa-solid fa-image" style={{marginRight:'8px'}}></i>Profile Photo
                        </label>
                        <input
                          type="file"
                          accept="image/png,image/jpeg,image/jpg,image/webp,image/gif"
                          onChange={(e) => setProfilePhotoFile(e.target.files?.[0] || null)}
                          style={{padding:'8px 12px',borderRadius:'8px',border:'1.5px solid #e0d5f5',fontSize:'13px'}}
                        />
                        <button
                          type="button"
                          ...
                          style={{marginTop:'8px',padding:'10px 16px',background:(!profilePhotoFile || profilePhotoUploading || profile.status !== 'approved')?'#ccc':'linear-gradient(135deg, rgb(15, 78, 168), rgb(0, 163, 163))',color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:600,cursor:(!profilePhotoFile || profilePhotoUploading || profile.status !== 'approved')?'not-allowed':'pointer'}}
                        >
                          {profilePhotoUploading ? 'Uploading...' : <span><i className="fa-solid fa-image"></i> Upload New Photo</span>}
                        </button>
                      </div>

                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',marginTop:'16px'}}>
                        <div className="ad-form-row">
                          <label><i className="fa-solid fa-phone" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Phone</label>
                          <input value={editData.phone} onChange={e => setEditData({...editData, phone: e.target.value})} style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                        </div>
                        <div className="ad-form-row">
                          <label><i className="fa-solid fa-location-dot" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Address</label>
                          <input value={editData.address||''} onChange={e => setEditData({...editData, address: e.target.value})} placeholder="e.g. Rajshahi, Bangladesh" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                        </div>
                      </div>

                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',marginTop:'16px'}}>
                        <div className="ad-form-row">
                          <label><i className="fa-solid fa-building" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Organization</label>
                          <input value={editData.company} onChange={e => setEditData({...editData, company: e.target.value})} style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                        </div>
                        <div className="ad-form-row">
                          <label><i className="fa-solid fa-briefcase" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Designation</label>
                          <input value={editData.designation||''} onChange={e => setEditData({...editData, designation: e.target.value})} style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                        </div>
                      </div>

                      <div className="ad-form-row" style={{marginTop:'16px'}}>
                        <label><i className="fa-solid fa-home" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Hall / Residential College</label>
                        <input value={editData.hall_name||''} onChange={e => setEditData({...editData, hall_name: e.target.value})} placeholder="e.g. Shahid Hall, Zia Hall" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'16px'}}>
                        <label><i className="fa-solid fa-calendar" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Current Job Start Date</label>
                        <input type="date" value={editData.current_job_start_date || ''} onChange={e => setEditData({...editData, current_job_start_date: e.target.value})} style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div style={{margin:'20px 0 4px',fontWeight:700,fontSize:'13px',color:'#0f4ea8',letterSpacing:'0.3px',textTransform:'uppercase'}}>
                        <i className="fa-solid fa-briefcase" style={{marginRight:'6px'}}></i>Past Work Experience
                      </div>
                      {(editData.past_jobs || []).map((job, idx) => (
                        <div className="ad-exp-item" key={`past-job-${idx}`} style={{background:'#f9f7fc',padding:'12px',borderRadius:'10px',marginBottom:'12px',border:'1px solid #e0d5f5'}}>
                          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                            <div>
                              <label style={{fontSize:'12px',fontWeight:600,color:'#666',display:'block',marginBottom:'4px'}}>Company</label>
                              <input
                                value={job.company || ''}
                                ...
                                style={{padding:'8px 12px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'13px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}}
                              />
                            </div>
                            <div>
                              <label style={{fontSize:'12px',fontWeight:600,color:'#666',display:'block',marginBottom:'4px'}}>Designation</label>
                              <input
                          ...
                          <button
                            ...
                            style={{marginTop:'8px',padding:'6px 12px',background:'#ffebee',color:'#c62828',border:'none',borderRadius:'6px',fontSize:'12px',fontWeight:600,cursor:'pointer',width:'100%'}}
                          >
                            <i className="fa-solid fa-trash"></i> Remove Experience
                          </button>
                        </div>
                      ))}
                      <button
                        ...
                        style={{width:'100%',padding:'10px',background:'#e3f2fd',color:'#0f4ea8',border:'2px dashed #0f4ea8',borderRadius:'8px',fontSize:'13px',fontWeight:600,cursor:'pointer',marginBottom:'16px'}}
                      >
                        <i className="fa-solid fa-plus"></i> Add Past Experience
                      </button>

                      <div className="ad-form-row">
                        <label><i className="fa-solid fa-graduation-cap" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Higher Study</label>
                        <input value={editData.higher_study||''} onChange={e => setEditData({...editData, higher_study: e.target.value})} placeholder="e.g. MSc in Data Science" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'16px'}}>
                        <label><i className="fa-solid fa-comment" style={{color:'#0f4ea8',marginRight:'6px'}}></i>About Me</label>
                        <textarea rows={3} value={editData.bio||''} onChange={e => setEditData({...editData, bio: e.target.value})} placeholder="A short description about yourself…" style={{resize:'vertical',fontFamily:'Inter,sans-serif',fontSize:'14px',padding:'10px 14px',border:'1.5px solid #e0d5f5',borderRadius:'10px',outline:'none',width:'100%',color:'#333'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'16px'}}>
                        <label><i className="fa-solid fa-microscope" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Research Interests</label>
                        <input value={editData.research_interests||''} onChange={e => setEditData({...editData, research_interests: e.target.value})} placeholder="e.g. Machine Learning, IoT, NLP" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'16px'}}>
                        <label><i className="fa-solid fa-star" style={{color:'#0f4ea8',marginRight:'6px'}}></i>Extracurricular Activities</label>
                        <input value={editData.extracurricular||''} onChange={e => setEditData({...editData, extracurricular: e.target.value})} placeholder="e.g. Debate Club, Programming Contest" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div style={{margin:'20px 0 12px',fontWeight:700,fontSize:'13px',color:'#0f4ea8',letterSpacing:'0.3px',textTransform:'uppercase'}}>
                        <i className="fa-solid fa-link" style={{marginRight:'6px'}}></i>Social Links
                      </div>

                      <div className="ad-form-row">
                        <label><i className="fa-brands fa-linkedin" style={{color:'#0077b5',marginRight:'6px'}}></i>LinkedIn</label>
                        <input value={editData.linkedin||''} onChange={e => setEditData({...editData, linkedin: e.target.value})} placeholder="https://linkedin.com/in/username" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'12px'}}>
                        <label><i className="fa-brands fa-github" style={{color:'#333',marginRight:'6px'}}></i>GitHub</label>
                        <input value={editData.github||''} onChange={e => setEditData({...editData, github: e.target.value})} placeholder="https://github.com/username" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'12px'}}>
                        <label><i className="fa-brands fa-twitter" style={{color:'#1da1f2',marginRight:'6px'}}></i>Twitter</label>
                        <input value={editData.twitter||''} onChange={e => setEditData({...editData, twitter: e.target.value})} placeholder="https://twitter.com/username" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-row" style={{marginTop:'12px'}}>
                        <label><i className="fa-solid fa-globe" style={{color:'#00a3a3',marginRight:'6px'}}></i>Website</label>
                        <input value={editData.website||''} onChange={e => setEditData({...editData, website: e.target.value})} placeholder="https://yourwebsite.com" style={{padding:'10px 14px',borderRadius:'10px',border:'1.5px solid #e0d5f5',fontSize:'14px',outline:'none',width:'100%',fontFamily:'Inter,sans-serif'}} />
                      </div>

                      <div className="ad-form-actions" style={{display:'flex',gap:'12px',marginTop:'24px'}}>
                        <button className="ad-btn-save" onClick={handleSaveProfile} style={{flex:1,padding:'12px 20px',background:'linear-gradient(135deg, rgb(15, 78, 168), rgb(0, 163, 163))',color:'white',border:'none',borderRadius:'12px',fontSize:'14px',fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px'}}>
                          <i className="fa-solid fa-floppy-disk"></i> Save Changes
                        </button>
                        <button className="ad-btn-cancel" onClick={() => setEditMode(false)} style={{flex:1,padding:'12px 20px',background:'#f0f0f0',color:'#333',border:'none',borderRadius:'12px',fontSize:'14px',fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px'}}>
          ...
          {activeView === 'events' && (
            <>
              <div className="ad-section-title" style={{justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'15px'}}>
                <span style={{display:'flex',alignItems:'center',gap:10}}>
                  <i className="fa-solid fa-calendar-days" style={{color:'#00a3a3'}}></i> Events
                  <span className="ad-badge-count">{events.filter(ev => ev.audience !== 'students' && (!eventViewSearch.trim() || ev.title.toLowerCase().includes(eventViewSearch.trim().toLowerCase()))).length}</span>
                </span>
                <span style={{position:'relative',display:'flex',alignItems:'center'}}>
                  <i className="fa-solid fa-magnifying-glass" style={{position:'absolute',left:10,color:'#00a3a3',fontSize:13,pointerEvents:'none'}}></i>
                  <input
                    ...
                    value={eventViewSearch}
                    onChange={e => setEventViewSearch(e.target.value)}
                    style={{paddingLeft:30,paddingRight:10,paddingTop:8,paddingBottom:8,border:'1.5px solid #d0f0f0',borderRadius:20,fontSize:13,outline:'none',fontFamily:'Inter,sans-serif',width:'200px',backgroundColor:'#f8fefe'}}
                  />
                </span>
              </div>
              <div className="ad-events-grid full">
                {events.filter(ev => ev.audience !== 'students' && (!eventViewSearch.trim() || ev.title.toLowerCase().includes(eventViewSearch.trim().toLowerCase()))).slice().sort((a, b) => b.id - a.id).map(ev => {
                  const bannerUrl = ev.banner_image_url || (ev.banner_image ? getUploadUrl(ev.banner_image) : null)
                  const attachmentHref = ev.attachment_url || (ev.attachment ? getUploadUrl(ev.attachment) : '')
                  return (
                  <div key={ev.id} className="ad-event-card">
                    ...
                    <div className="ad-event-content">
                      <div className="ad-event-date">
                        <i className="fa-solid fa-calendar"></i> {formatDateFull(ev.date)}
                        {ev.time && <div className="ad-event-time"><i className="fa-solid fa-clock"></i> {formatTime12Hour(ev.time)}</div>}
                      </div>
                      <h4>{ev.title}</h4>
                      ...
                      <div style={{display:'flex', gap:8, marginTop:12}}>
                        {ev.audience !== 'students' && (() => {
                          ...
                          return (
                          <button
                            className={`ad-btn-join${registeredEventIds.includes(ev.id) ? ' joined' : ''}${isDeadlinePassed ? ' expired' : ''}`}
                            onClick={() => !registeredEventIds.includes(ev.id) && !isDeadlinePassed && openRegisterModal(ev)}
                            disabled={registeredEventIds.includes(ev.id) || isDeadlinePassed}
                            style={{flex:1}}
                          >
                            {registeredEventIds.includes(ev.id)
                              ? <span><i className="fa-solid fa-check"></i> Registered</span>
                              : isDeadlinePassed
                              ? <span><i className="fa-solid fa-ban"></i> Expired</span>
                              : <span><i className="fa-solid fa-pen-to-square"></i> Register</span>
                            }
                          </button>
                          )
                        })()}
          ...
          {activeView === 'trainings' && (
            <>
              ...
              <div className="ad-training-grid">
                {trainings.map(tr => {
                  const expired = isTrainingExpired(tr)
                  const statusText = expired ? 'Expired' : tr.status
                  const statusClass = expired ? 'expired' : (tr.status === 'Full' ? 'full' : 'upcoming')
                  const enrollBlocked = !enrolledTrainingIds.includes(tr.id) && (tr.status === 'Full' || expired)
                  const pct = Math.round((tr.enrolled / tr.seats) * 100)
                  return (
                    <div key={tr.id} className="ad-training-card">
                      <div className="ad-training-header">
                        <h4>{tr.title}</h4>
                        <span className={`ad-train-status ${statusClass}`}>{statusText}</span>
                      </div>
                      <p><i className="fa-solid fa-user-tie"></i> {tr.trainer}</p>
                      <p><i className="fa-solid fa-calendar"></i> {tr.date}</p>
                      {tr.time && <p><i className="fa-solid fa-clock"></i> {formatTime12Hour(tr.time)}</p>}
                      ...
                      <button
                        className={`ad-btn-enroll${enrolledTrainingIds.includes(tr.id) ? ' enrolled' : ''}${enrollBlocked ? ' disabled' : ''}`}
                        onClick={() => !enrolledTrainingIds.includes(tr.id) && !enrollBlocked && openTrainEnrollModal(tr)}
                        disabled={enrollBlocked}
                      >
                        {enrolledTrainingIds.includes(tr.id)
                          ? <span><i className="fa-solid fa-check"></i> Enrolled</span>
                          : expired
                            ? 'Expired'
                            : tr.status === 'Full'
                            ? 'Seats Full'
                            : <span><i className="fa-solid fa-plus"></i> Enroll</span>
                        }
                      </button>
                      {isTrainingCreator(tr) && (
                        <button
                      ...
                      <form onSubmit={handleSubmitJob} style={{padding:'24px 28px', display:'flex', flexDirection:'column', gap:14}}>
                        ...
                        <div style={{display:'flex', gap:10, marginTop:4}}>
                          <button type="submit" disabled={jobSubmitting} style={{
                            ...
                            {jobSubmitting ? 'Submitting…' : <span><i className="fa-solid fa-paper-plane"></i> Submit for Review</span>}
                          </button>
          ...
          {/* ══ SUCCESS STORY ══ */}
          {activeView === 'success-story' && (
            <SuccessStory currentAlumni={profile} />
          )}
              ...
              <div style={{display:'flex', gap:10, marginTop:4}}>
                <button type="submit" disabled={registerSubmitting} style={{
                  ...
                  {registerSubmitting
                    ...
                    : <span><i className="fa-solid fa-check"></i> Submit Registration</span>
                  }
                </button>
              ...
              <div style={{display:'flex', gap:10, marginTop:4}}>
                <button type="submit" disabled={trainEnrollSubmitting} style={{
                  ...
                  {trainEnrollSubmitting
                    ? <span><i className="fa-solid fa-spinner fa-spin"></i> Submitting…</span>
                    : <span><i className="fa-solid fa-check"></i> Submit Enrollment</span>
                  }
                </button>
              ...
              <div style={{display:'flex', gap:10, marginTop:4}}>
                <button type="submit" disabled={addTrainSubmitting} style={{
                  ...
                  {addTrainSubmitting
                    ...
                    : <span><i className="fa-solid fa-plus"></i> Add Training</span>
                  }
                </button>
