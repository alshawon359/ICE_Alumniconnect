...
import { ProfileSectionCard, TagCloud, TimelineList } from '../components/profile/ProfileBlocks'
...
import { buildTimelineEntries, splitTags } from '../utils/profile-utils'
...
export default function StudentDashboard() {
  ...
  const profileAvatarUrl = resolveAvatarUrl(profile)
  ...
  const profileTimelineEntries = useMemo(() => buildTimelineEntries(profile), [profile])
  const profileExpertiseTags = useMemo(() => splitTags(profile.research_interests), [profile.research_interests])
  const profileActivityTags = useMemo(() => splitTags(profile.extracurricular), [profile.extracurricular])
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem('student-force-dashboard-once') === '1') {
        setActiveView('dashboard')
        sessionStorage.removeItem('student-force-dashboard-once')
      }
    } catch {}
  }, [])
  ...
  return (
    ...
    <div className="ad-wrapper">
      ...
      <div className="ad-main">
        ...
        <div className="ad-content" data-scroll-container="student-dashboard-content">
          ...
          {activeView === 'dashboard' && (
            <>
                ...
                <div style={{background:'white',borderRadius:22,width:'100%',maxWidth:480,boxShadow:'0 16px 60px rgba(0,0,0,0.25)',overflow:'hidden'}}>
                    ...
                    <div style={{padding:'24px 26px'}}>
                      ...
                      <div style={{display:'flex',gap:10}}>
                        ...
                        >
                          {upgradeSubmitting ? 'Submitting…' : <span><i className="fa-solid fa-paper-plane" style={{marginRight:8}}></i>Submit Request</span>}
                        </button>
                        <button
              ...
              <div className="ad-profile-card">
                ...
                {/* Removed 'Open My Profile' button per request */}
              </div>

              {/* Upcoming events */}
              <div className="ad-section-title" style={{justifyContent:'space-between',flexWrap:'wrap'}}>
                <span style={{display:'flex',alignItems:'center',gap:10}}>
                  <i className="fa-solid fa-calendar-days" style={{color:'#a4508b'}}></i> Events
                  ...
                  >
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:13,minWidth:0,paddingRight:isCompactDirectory ? 0 : 12,marginBottom:isCompactDirectory ? 10 : 0}}>
                      <div style={{
                        ...
                        {resolveAvatarUrl(a)
                          ? <img src={resolveAvatarUrl(a)} alt={a.name} style={{width:'100%',height:'100%',objectFit:'cover',cursor:'zoom-in',display:'block'}} onClick={() => openImagePreview(resolveAvatarUrl(a), a.name)} />
                          : <span>{a.name[0].toUpperCase()}</span>
                        }
                      </div>
                      <div style={{minWidth:0}}>
          ...
          {activeView === 'profile' && (
            <>
              <div style={{background:'linear-gradient(135deg,#1a6eb5,#4aa3e0)',borderRadius:24,padding:'24px',marginBottom:24,boxShadow:'0 12px 40px rgba(26,110,181,0.20)',color:'white'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:18,flexWrap:'wrap'}}>
                  <div style={{display:'flex',alignItems:'center',gap:18,minWidth:0}}>
                    <div style={{width:84,height:84,borderRadius:'50%',overflow:'hidden',background:'rgba(255,255,255,0.18)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,boxShadow:'0 10px 24px rgba(0,0,0,0.16)'}}>
                      {(profileAvatarUrl && !avatarLoadFailed)
                        ? <img src={profileAvatarUrl} alt={profile.name} style={{width:'100%',height:'100%',objectFit:'cover'}} onError={() => setAvatarLoadFailed(true)} onClick={() => openImagePreview(profileAvatarUrl, profile.name)} />
                        : <span style={{fontSize:30,fontWeight:800}}>{(profile.name || 'U').charAt(0).toUpperCase()}</span>
                      }
                    </div>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:700,letterSpacing:'0.5px',textTransform:'uppercase',opacity:0.85}}>Student Profile</div>
                      <h2 style={{margin:'6px 0 4px',fontSize:28,lineHeight:1.15}}>{profile.name || 'Unnamed Student'}</h2>
                      <div style={{display:'flex',flexWrap:'wrap',gap:10,fontSize:13,opacity:0.92}}>
                        <span>{profile.department || 'ICE'}</span>
                        <span>•</span>
                        <span>{profile.session || 'Session not set'}</span>
                        <span>•</span>
              ...
              <section className="profile-grid profile-grid--three" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:20,marginBottom:20}}>
                <ProfileSectionCard title="Academic Identity" eyebrow="Student details">
                  <dl className="profile-definition-list">
                    <div><dt>Department</dt><dd>{profile.department || 'ICE'}</dd></div>
                    <div><dt>Session</dt><dd>{profile.session || '—'}</dd></div>
                    <div><dt>Student ID</dt><dd>{profile.student_id || '—'}</dd></div>
                    <div><dt>Hall Name</dt><dd>{profile.hall_name || '—'}</dd></div>
                  </dl>
                </ProfileSectionCard>

                <ProfileSectionCard title="Contact & Links" eyebrow="Reach out">
                  <dl className="profile-definition-list profile-definition-list--compact">
          ...
          {activeView === 'events' && (
            <>
              <div className="ad-section-title" style={{justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'15px'}}>
                <span style={{display:'flex',alignItems:'center',gap:10}}>
                  <i className="fa-solid fa-calendar-days" style={{color:'#a4508b'}}></i> Events
                  <span className="ad-badge-count">{events.filter(ev => ev.audience !== 'alumni' && (!eventViewSearch.trim() || ev.title.toLowerCase().includes(eventViewSearch.trim().toLowerCase()))).length}</span>
                </span>
                <span style={{position:'relative',display:'flex',alignItems:'center'}}>
                  <i className="fa-solid fa-magnifying-glass" style={{position:'absolute',left:10,color:'#a4508b',fontSize:13,pointerEvents:'none'}}></i>
                  <input
                    ...
                    style={{paddingLeft:30,paddingRight:10,paddingTop:8,paddingBottom:8,border:'1.5px solid #e8d5f2',borderRadius:20,fontSize:13,outline:'none',fontFamily:'Inter,sans-serif',width:'200px',backgroundColor:'#faf8fc'}}
                  />
                </span>
              </div>
              <div className="ad-events-grid full">
                {events.filter(ev => ev.audience !== 'alumni' && (!eventViewSearch.trim() || ev.title.toLowerCase().includes(eventViewSearch.trim().toLowerCase()))).slice().sort((a, b) => b.id - a.id).map(ev => {
                  const bannerUrl = ev.banner_image_url || (ev.banner_image ? getUploadUrl(ev.banner_image) : null)
                  const attachmentHref = ev.attachment_url || (ev.attachment ? getUploadUrl(ev.attachment) : '')
                  return (
                  <div key={ev.id} className="ad-event-card">
