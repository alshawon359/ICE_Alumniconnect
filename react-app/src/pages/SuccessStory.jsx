...
const SuccessStory = ({ currentAlumni }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        response = await submitSuccessStory(payload)
      }
  ...
  const fetchMyStories = async () => {
    try {
      ...
      const owner = getCurrentOwnerPayload()
      const res = await getUserSuccessStories(owner.alumni_id, owner.student_id)
      if (res?.ok && res?.data?.success) {
        const fetchedStories = Array.isArray(res.data.data) ? res.data.data : []
        setMyStories(fetchedStories.length > 0 ? fetchedStories : getOwnStoriesFromFeed())
      } else {
        console.error('Failed to fetch my stories', res)
