...
export const getAdminAccounts = () => request('/admin-accounts', { headers: getAdminAuthHeaders() });
export const createAdminAccount = (payload) => request('/admin-accounts', {
  method: 'POST',
  headers: getAdminAuthHeaders(),
  body: JSON.stringify(payload || {}),
});
export const deleteAdminAccount = (adminId) => request(`/admin-accounts/${adminId}`, {
  method: 'DELETE',
  headers: getAdminAuthHeaders(),
});
export const updateAdminAccountName = (adminId, name) => request(`/admin-accounts/${adminId}`, {
  method: 'PATCH',
  headers: getAdminAuthHeaders(),
...
export const approveAlumni   = (id)      => request(`/approve/${id}`,  { method:'POST', headers: getAdminAuthHeaders() });
export const rejectAlumni    = (id)      => request(`/reject/${id}`,   { method:'POST', headers: getAdminAuthHeaders() });
...
export const approveReferral = (id) => request(`/referrals/${id}/approve`, { method:'POST', headers: getAdminAuthHeaders() });
export const rejectReferral = (id, admin_note='') => request(`/referrals/${id}/reject`, { method:'POST', headers: getAdminAuthHeaders(), body: JSON.stringify({ admin_note }) });
...
export const approveUpgrade   = (id)     => request(`/approve-upgrade/${id}`,  { method:'POST', headers: getAdminAuthHeaders() });
export const rejectUpgrade    = (id)     => request(`/reject-upgrade/${id}`,   { method:'POST', headers: getAdminAuthHeaders() });
...
export const uploadEventAttachment = (eid, file) => {
  const formData = new FormData();
  formData.append('attachment', file);
  return request(`/events/${eid}/upload-attachment`, { method: 'POST', body: formData }).then(async (res) => {
    if (res && res.status === 404 && typeof window !== 'undefined') {
      try {
        const fallbackUrl = `${window.location.origin}/events/${eid}/upload-attachment`;
        const resp = await fetch(fallbackUrl, { method: 'POST', body: formData });
        const data = await resp.json().catch(() => ({ success: false, message: `Upload failed (HTTP ${resp.status})` }));
        return { ok: resp.ok, status: resp.status, data };
      } catch (err) {
...
export const getStoryReactions = (storyId) => {
  return request(`/success-stories/${encodeURIComponent(storyId)}/reactions`);
};

export const postStoryComment = (storyId, payload) => {
  return request(`/success-stories/${encodeURIComponent(storyId)}/comments`, { method: 'POST', body: JSON.stringify(payload || {}) });
};

export const getStoryComments = (storyId) => {
  return request(`/success-stories/${encodeURIComponent(storyId)}/comments`);
};
...
export const getTrainingAttendees   = (tid, alumniId) => request(`/trainings/${tid}/attendees${alumniId ? `?alumni_id=${encodeURIComponent(alumniId)}` : ''}`);
